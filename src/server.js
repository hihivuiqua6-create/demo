...
// ---------- ANTI-CRASH / RENDER STABILITY ----------
// Bắt lỗi nền để bot không tắt cả server vì 1 callback/API lỗi lẻ.
process.on("unhandledRejection", (err) => {
  console.error("[ANTI-CRASH] unhandledRejection:", err && (err.stack || err.message) || err);
});
process.on("uncaughtException", (err) => {
  console.error("[ANTI-CRASH] uncaughtException:", err && (err.stack || err.message) || err);
});
...
function safeSetInterval(fn, ms) {
  const t = setInterval(async () => {
    try { await fn(); } catch (e) { console.error("[interval]", e.message || e); }
  }, ms);
  if (t.unref) t.unref();
  return t;
}
async function safeStopPolling(bot) {
  try { await bot.stopPolling({ cancel: true }); } catch {}
}
async function safeStartPolling(bot) {
  try { await bot.startPolling({ restart: true }); } catch (e) { console.error("[start polling]", e.message || e); }
}
function pollingOptions() {
  return { polling: { interval: 900, autoStart: true, params: { timeout: 15 } } };
}
function isNetOrTelegramCrash(e) {
  const msg = String((e && (e.code || e.message)) || e || "");
  return /409|ETELEGRAM|ECONNRESET|ETIMEDOUT|EFATAL|EAI_AGAIN|ENOTFOUND|socket hang up|aborted/i.test(msg);
}
function attachPollingRecovery(bot, label) {
  let restarting = false;
  let retry = 0;
  bot.on("polling_error", async (e) => {
    const msg = e && (e.code || e.message) || e;
    console.error(`[${label}] polling_error`, msg);
    if (!isNetOrTelegramCrash(e) || restarting) return;
    restarting = true;
    retry = Math.min(retry + 1, 6);
    await safeStopPolling(bot);
    setTimeout(async () => {
      await safeStartPolling(bot);
      restarting = false;
    }, 1500 * retry);
  });
  bot.on("webhook_error", (e) => console.error(`[${label}] webhook_error`, e && (e.message || e)));
}
...
// Render free dễ sập nếu chạy đủ cả 3 loại bot cùng lúc.
// Không giới hạn số bot con. Chỉ giới hạn số LOẠI bot đang bật: tối đa 2 loại
// Ví dụ được: nhiều Mini App + nhiều Bot Rải. Không được: Mini App + Bot Rải + Bot Bán Acc.
const MAX_ACTIVE_BOT_TYPES = Number(process.env.MAX_ACTIVE_BOT_TYPES || 2);
function getActiveBotTypes(exceptType = '', exceptId = 0) {
  const types = new Set();
  const miniSql = 'SELECT COUNT(*) c FROM bots WHERE active=1' + (exceptType === 'mini' ? ' AND id<>?' : '');
  const arSql = 'SELECT COUNT(*) c FROM autorai_bots WHERE active=1' + (exceptType === 'autorai' ? ' AND id<>?' : '');
  const accSql = 'SELECT COUNT(*) c FROM accshop_bots WHERE active=1' + (exceptType === 'accshop' ? ' AND id<>?' : '');
  const mini = db.prepare(miniSql).get(...(exceptType === 'mini' ? [exceptId] : [])).c || 0;
  const ar = db.prepare(arSql).get(...(exceptType === 'autorai' ? [exceptId] : [])).c || 0;
  const acc = db.prepare(accSql).get(...(exceptType === 'accshop' ? [exceptId] : [])).c || 0;
  if (mini > 0) types.add('mini');
  if (ar > 0) types.add('autorai');
  if (acc > 0) types.add('accshop');
  return types;
}
function assertCanEnableBot(type, id = 0) {
  const types = getActiveBotTypes(type, Number(id) || 0);
  types.add(type);
  if (types.size > MAX_ACTIVE_BOT_TYPES) {
    const err = new Error(`Chỉ được bật tối đa ${MAX_ACTIVE_BOT_TYPES} dạng bot cùng lúc. Có thể bật nhiều bot trong cùng dạng, nhưng hãy tắt bớt 1 dạng khác trước.`);
    err.status = 400;
    throw err;
  }
}
...
async function startBot(row) {
  ...
  try {
    const main = new TelegramBot(fresh.token, pollingOptions());
    attachPollingRecovery(main, `mini #${fresh.id}`);
  ...
  const arBot = arToken ? new TelegramBot(arToken, pollingOptions()) : entry.main;
  if (arToken && arBot) attachPollingRecovery(arBot, `autoreply #${fresh.id}`);
...
// Dọn log cũ để DB/RAM không phình làm Render free bị kill.
safeSetInterval(() => {
  try { db.prepare("DELETE FROM bot_messages WHERE id NOT IN (SELECT id FROM bot_messages ORDER BY id DESC LIMIT 5000)").run(); } catch {}
  try { db.prepare("DELETE FROM accshop_activity WHERE id NOT IN (SELECT id FROM accshop_activity ORDER BY id DESC LIMIT 5000)").run(); } catch {}
}, 30 * 60 * 1000);
...
async function startAutoraiBot(row) {
  ...
  try {
    const bot = new TelegramBot(row.token, pollingOptions());
    attachPollingRecovery(bot, `autorai #${row.id}`);
    try {
...
safeSetInterval(() => enforceExpiry().catch(e => console.error("[expiry]", e.message)), 60 * 1000);
enforceExpiry().catch(() => {});
...
app.use(express.json({ limit: process.env.JSON_MAX || "12mb" }));
app.use(express.urlencoded({ extended: true, limit: process.env.JSON_MAX || "12mb" }));
app.get(["/", "/healthz"],(req,res)=>res.json({ok:true, uptime:process.uptime(), memory:process.memoryUsage().rss, live:{mini:liveBots.size, autorai:liveAutorai.size, accshop:liveAccShop.size}, time:new Date().toISOString()}));
...
app.patch("/api/bots/:id", anyAuth, async (req, res) => {
  ...
  if (active !== undefined) {
    if (active) { try { assertCanEnableBot('mini', id); } catch(e){ return res.status(e.status||400).json({error:e.message}); } }
...
app.patch("/api/autorai/:id", anyAuth, async (req, res) => {
  ...
  if (active !== undefined) {
    if (active) { try { assertCanEnableBot('autorai', id); } catch(e){ return res.status(e.status||400).json({error:e.message}); } }
...
async function startAccShopBot(row){
  ...
  const bot=new TelegramBot(row.token,pollingOptions());
  const state=new Map();
  const cbThrottle=new Map(); liveAccShop.set(row.id,{bot,state});
  ...
  bot.on('polling_error',e=>{ const msg=e.code||e.message; console.error(`[accshop #${row.id}]`,msg); if(String(msg).includes('409')) bot.stopPolling().catch(()=>{}); });
  ...
  function parseAccBody(body){
    ...
    const status=a[3]||'on';
    const cat=a[4]||'';
    // Mẫu mới: Tên|Giá|Mô tả|Trạng thái|Danh mục|user|pass
    // Vì user|pass cũng dùng dấu | nên ghép lại toàn bộ phần sau danh mục.
    const secret=a.length>=7 ? a.slice(5).join('|').trim() : (a[5]||'').trim();
...
Chọn duyệt nếu đã nhận tiền.`,{reply_markup:{inline_keyboard:[[{text:'✅ Duyệt + cộng tiền',callback_data:`accpay:ok:${topupId}`},{text:'❌ Từ chối',callback_data:`accpay:no:${topupId}`}],[{text:'👤 Xem UID',callback_data:`accpay:user:${uid}`}]]}}); return; }
      ...
      if(cmd==='start'){ await sendUserMenu(chat,cur); return; }
      ...
      if(cmd==='addacc'||cmd==='themacc'){ if(arg){ const p=parseAccBody(arg); const c=findCat(p.cat); db.prepare('INSERT INTO accshop_accounts(bot_id,category_id,title,description,secret,price,status) VALUES(?,?,?,?,?,?,?)').run(row.id,c.id,p.title,p.desc,p.secret||p.desc,p.price,p.status); await bot.sendMessage(chat,`✅ Đã thêm 1 acc vào ${c.name}.`); } else { state.set(uid,'wait_addacc'); await bot.sendMessage(chat,'📦 Gửi acc theo mẫu, có thể gửi kèm ảnh:\n<code>Tên acc|Giá|Mô tả|Trạng thái|Danh mục|user|pass</code>\nVD: Acc Rank Huyền Thoại|5000|Chưa band ID|on|Acc Clone LV5|user1|pass1',{parse_mode:'HTML'}); } return; }
...
Đã giao tự động.`); return; }
      ...
      if(data.startsWith('accadmin:')){ if(!isAccAdmin(cur,uid)) return; const a=data.split(':')[1];
        ...
        if(a==='addacc'){ state.set(uid,'wait_addacc'); return bot.sendMessage(chat,'📦 Gửi mẫu: <code>Tên acc|Giá|Mô tả|Trạng thái|Danh mục|user|pass</code>\nCó thể gửi kèm ảnh.',{parse_mode:'HTML'}); }
...
app.post('/api/accshop', adminOnly, async (req,res)=>{ const {token,owner_seller_id,bank_name,bank_account,bank_owner,bank_template,support_text,support_image_url,menu_note}=req.body||{}; if(!/^\d+:[A-Za-z0-9_-]{30,}$/.test(token||'')) return res.status(400).json({error:'Token sai định dạng'}); try{ assertCanEnableBot('accshop', 0); const key=accKey(); const info=db.prepare('INSERT INTO accshop_bots(token,owner_seller_id,admin_key,bank_name,bank_account,bank_owner,bank_template,support_text,support_image_url,menu_note) VALUES(?,?,?,?,?,?,?,?,?,?)').run(token,owner_seller_id?Number(owner_seller_id):null,key,bank_name||'',bank_account||'',bank_owner||'',bank_template||'NAP{random}', support_text||'', support_image_url||'', menu_note||''); const row=db.prepare('SELECT * FROM accshop_bots WHERE id=?').get(info.lastInsertRowid); await startAccShopBot(row); res.json({id:row.id,admin_key:key}); }catch(e){res.status(400).json({error:e.message})} });
app.patch('/api/accshop/:id', anyAuth, async (req,res)=>{ const id=Number(req.params.id); if(!canTouchAccShop(req,id)) return res.status(403).json({error:'Không có quyền'}); const b=req.body||{}; if(req.user.role==='admin'){ for(const k of ['bank_name','bank_account','bank_owner','bank_template','support_text','support_image_url','menu_note']) if(b[k]!==undefined) db.prepare(`UPDATE accshop_bots SET ${k}=? WHERE id=?`).run(String(b[k]||''),id); if(b.owner_seller_id!==undefined) db.prepare('UPDATE accshop_bots SET owner_seller_id=? WHERE id=?').run(b.owner_seller_id?Number(b.owner_seller_id):null,id); if(b.regen_key){ const k=accKey(); db.prepare('UPDATE accshop_bots SET admin_key=? WHERE id=?').run(k,id); } }
 if(b.active!==undefined){ if(b.active){ try{ assertCanEnableBot('accshop', id); }catch(e){ return res.status(e.status||400).json({error:e.message}); } } db.prepare('UPDATE accshop_bots SET active=? WHERE id=?').run(b.active?1:0,id); if(b.active) await restartAccShopBot(id); else await stopAccShopBot(id); } res.json({ok:true}); });
...
// ---------- MAIN ADMIN BOT SWITCHES ----------
app.get('/api/admin/bot-switches', adminOnly, (req,res)=>{
  const mini = db.prepare(`SELECT id, bot_username, active, created_at FROM bots ORDER BY id DESC`).all()
    .map(x=>({type:'mini', id:x.id, name:'@'+(x.bot_username||'?'), label:'Mini App', active:!!x.active, live:liveBots.has(x.id), created_at:x.created_at}));
  const ar = db.prepare(`SELECT id, bot_username, active, running, created_at FROM autorai_bots ORDER BY id DESC`).all()
    .map(x=>({type:'autorai', id:x.id, name:'@'+(x.bot_username||'?'), label:'Auto-Rải', active:!!x.active, live:liveAutorai.has(x.id), running:!!x.running, created_at:x.created_at}));
  const acc = db.prepare(`SELECT id, bot_username, active, created_at FROM accshop_bots ORDER BY id DESC`).all()
    .map(x=>({type:'accshop', id:x.id, name:'@'+(x.bot_username||'?'), label:'Bán Acc', active:!!x.active, live:liveAccShop.has(x.id), created_at:x.created_at}));
  const items=[...mini,...ar,...acc].sort((a,b)=>String(b.created_at||'').localeCompare(String(a.created_at||'')));
  res.json({max:MAX_ACTIVE_BOTS, active_count:activeBotCount(), items});
});
app.patch('/api/admin/bot-switches/:type/:id', adminOnly, async (req,res)=>{
  const type=String(req.params.type||''); const id=Number(req.params.id); const on=!!req.body?.active;
  try{
    if(on) assertCanEnableBot(type,id);
    if(type==='mini'){
      const r=db.prepare('SELECT * FROM bots WHERE id=?').get(id); if(!r) return res.status(404).json({error:'Không tìm thấy bot'});
      db.prepare('UPDATE bots SET active=? WHERE id=?').run(on?1:0,id); if(on) await startBot(db.prepare('SELECT * FROM bots WHERE id=?').get(id)); else await stopBot(id);
    } else if(type==='autorai'){
      const r=db.prepare('SELECT * FROM autorai_bots WHERE id=?').get(id); if(!r) return res.status(404).json({error:'Không tìm thấy bot'});
      db.prepare('UPDATE autorai_bots SET active=? WHERE id=?').run(on?1:0,id); if(on) await startAutoraiBot(db.prepare('SELECT * FROM autorai_bots WHERE id=?').get(id)); else await stopAutoraiBot(id);
    } else if(type==='accshop'){
      const r=db.prepare('SELECT * FROM accshop_bots WHERE id=?').get(id); if(!r) return res.status(404).json({error:'Không tìm thấy bot'});
      db.prepare('UPDATE accshop_bots SET active=? WHERE id=?').run(on?1:0,id); if(on) await restartAccShopBot(id); else await stopAccShopBot(id);
    } else return res.status(400).json({error:'Loại bot không hợp lệ'});
    res.json({ok:true, active_count:activeBotCount(), max:MAX_ACTIVE_BOTS});
...
async function enforceStartupBotLimit(){
  try{
    const rows=[
      ...db.prepare("SELECT 'mini' type,id,created_at FROM bots WHERE active=1").all(),
      ...db.prepare("SELECT 'autorai' type,id,created_at FROM autorai_bots WHERE active=1").all(),
      ...db.prepare("SELECT 'accshop' type,id,created_at FROM accshop_bots WHERE active=1").all(),
    ].sort((a,b)=>String(b.created_at||'').localeCompare(String(a.created_at||'')));
    const keep=new Set(rows.slice(0,MAX_ACTIVE_BOTS).map(x=>x.type+':'+x.id));
    for(const x of rows.slice(MAX_ACTIVE_BOTS)){
      if(x.type==='mini'){ db.prepare('UPDATE bots SET active=0 WHERE id=?').run(x.id); await stopBot(x.id); }
      if(x.type==='autorai'){ db.prepare('UPDATE autorai_bots SET active=0 WHERE id=?').run(x.id); await stopAutoraiBot(x.id); }
      if(x.type==='accshop'){ db.prepare('UPDATE accshop_bots SET active=0 WHERE id=?').run(x.id); await stopAccShopBot(x.id); }
    }
    if(rows.length>MAX_ACTIVE_BOTS) console.log(`⚙️  Bot limit: giữ ${MAX_ACTIVE_BOTS} bot mới nhất, tắt ${rows.length-MAX_ACTIVE_BOTS} bot dư.`);
  }catch(e){ console.error('[bot limit]', e.message); }
}

async function gracefulShutdown(sig){
  console.log(`[shutdown] ${sig}`);
  try { for (const [id] of liveBots) await stopBot(id); } catch {}
  try { for (const [id] of liveAutorai) await stopAutoraiBot(id); } catch {}
  try { for (const [id] of liveAccShop) await stopAccShopBot(id); } catch {}
  try { db.close(); } catch {}
  process.exit(0);
}
