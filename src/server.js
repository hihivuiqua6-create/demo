...
async function startAccShopBot(row){
  ...
  const bot=new TelegramBot(row.token,{polling:{interval:800,autoStart:true,params:{timeout:20}}});
  ...
  function userKeyboard(){ return {inline_keyboard:[
    [{text:'🛍️ Mua Key/Acc',callback_data:'acc:kho'},{text:'💳 Nạp Tiền',callback_data:'acc:nap'}],
  ...
  function adminKeyboard(){ return {inline_keyboard:[
    [{text:'📦 Thêm acc',callback_data:'accadmin:themacc'},{text:'📢 Gửi thông báo',callback_data:'accadmin:thongbao'}],
    [{text:'🏦 Cấu hình bank',callback_data:'accadmin:bank'},{text:'👮 List admin',callback_data:'accadmin:listadmin'}],
    [{text:'🛍️ Xem kho user',callback_data:'acc:kho'},{text:'💎 Menu user',callback_data:'acc:start'}]
  ]}; }
  async function sendUserMenu(chat, cur){
    const note = cur.menu_note ? '\n\n📝 <b>Lưu ý:</b>\n' + escapeHtml(cur.menu_note) : '\n\n📝 <b>Lưu ý:</b> Bấm nút bên dưới để thao tác nhanh. Lệnh user được ghim sạch, admin dùng lệnh ẩn.';
  ...
  async function sendAdminMenu(chat){
    return bot.sendMessage(chat,'✅ <b>Đăng nhập admin thành công.</b>\n\n🛡️ <b>BẢNG LỆNH ADMIN ẨN</b>\n━━━━━━━━━━━━━━━━━━━━\n<code>/themacc</code> — thêm acc bằng mô tả + ảnh\n<code>/thongbao nội_dung</code> — gửi toàn bộ user\n<code>/bank BANK|STK|CHỦ TK|NAP{random}</code> — cấu hình VietQR\n<code>/listadmin</code> — xem admin\n\n📌 Admin có thể bấm nút bên dưới để hiện mẫu lệnh nhanh.',{parse_mode:'HTML',reply_markup:adminKeyboard()});
  }
  async function sendSupport(chat, cur){
    ...
    if(img) return bot.sendPhoto(chat,img,{caption:escapeHtml(txt),parse_mode:'HTML'}).catch(()=>bot.sendMessage(chat,escapeHtml(txt),{parse_mode:'HTML'}));
    return bot.sendMessage(chat,escapeHtml(txt),{parse_mode:'HTML'});
  }
  bot.on('message',async msg=>{
    try{
      ...
      if(st==='wait_topup' && /^\d{4,}$/.test(text)){ const amount=Math.max(1000,Number(text)); const code=(cur.bank_template||'NAP{random}').replace('{random}',crypto.randomBytes(3).toString('hex').toUpperCase()).replace('{uid}',String(uid)); db.prepare('INSERT INTO accshop_topups(bot_id,tg_user_id,amount,code) VALUES(?,?,?,?)').run(row.id,uid,amount,code); state.delete(uid); const qr=vietQr(cur,amount,code); const cap=`💳 <b>TẠO ĐƠN NẠP</b>\n━━━━━━━━━━━━━━\nSố tiền: <b>${vn(amount)}đ</b>\nBank: <b>${escapeHtml(cur.bank_name)}</b>\nSTK: <code>${escapeHtml(cur.bank_account)}</code>\nTên TK: <b>${escapeHtml(cur.bank_owner)}</b>\nNội dung: <code>${escapeHtml(code)}</code>`; if(qr) await bot.sendPhoto(chat,qr,{caption:cap,parse_mode:'HTML'}); else await bot.sendMessage(chat,cap+'\n⚠️ Admin chưa cấu hình bank/VietQR.',{parse_mode:'HTML'}); await notifyAdmins(`🔔 User <b>${escapeHtml(msg.from.first_name||msg.from.username||uid)}</b> vừa tạo đơn nạp <b>${vn(amount)}đ</b>\nNội dung: <code>${escapeHtml(code)}</code>`); return; }
      if(st==='wait_acc' && isAccAdmin(cur,uid)){ const body=text||msg.caption||''; if(!body && !msg.photo) return; const payload={title:'Acc game',desc:'',secret:'',price:0}; for(const line of body.split(/\n+/)){ const [k,...v]=line.split(':'); const val=v.join(':').trim(); if(!val) continue; const key=k.toLowerCase(); if(key.includes('tên')||key.includes('ten')||key==='title') payload.title=val; else if(key.includes('mô')||key.includes('mo')||key.includes('desc')) payload.desc=val; else if(key.includes('giá')||key.includes('gia')||key.includes('price')) payload.price=Number(val.replace(/\D/g,''))||0; else if(key.includes('acc')||key.includes('tk')||key.includes('login')||key.includes('secret')) payload.secret=val; } const img=msg.photo?.length?msg.photo[msg.photo.length-1].file_id:null; db.prepare('INSERT INTO accshop_accounts(bot_id,title,description,image_file_id,secret,price) VALUES(?,?,?,?,?,?)').run(row.id,payload.title,payload.desc,img,payload.secret,payload.price); state.delete(uid); await bot.sendMessage(chat,'✅ Đã thêm acc vào kho.'); return; }
      ...
      if(['admin','themacc','thongbao','bank','listadmin'].includes(cmd) && !isAccAdmin(cur,uid)){ return; }
      if(cmd==='admin'){ await sendAdminMenu(chat); return; }
      if(cmd==='themacc'){ state.set(uid,'wait_acc'); await bot.sendMessage(chat,'📦 Gửi mô tả acc theo mẫu, có thể kèm ảnh:\nTên: ...\nGiá: 10000\nMô tả: ...\nAcc: user|pass'); return; }
  ...
  bot.on('callback_query', async q=>{
    try{
      ...
      if(data==='acc:nap'){ state.set(uid,'wait_topup'); return bot.sendMessage(chat,'💳 <b>NẠP TIỀN VIETQR</b>\n━━━━━━━━━━━━━━\nNhập số tiền muốn nạp. VD: <code>50000</code>\n\n⚠️ Chuyển khoản đúng nội dung để admin kiểm tra nhanh.',{parse_mode:'HTML'}); }
      if(data==='acc:kho'){ const accs=db.prepare('SELECT * FROM accshop_accounts WHERE bot_id=? AND sold=0 ORDER BY id DESC LIMIT 30').all(row.id); if(!accs.length) return bot.sendMessage(chat,'📦 Kho acc đang trống.'); await bot.sendMessage(chat,`🛍️ <b>DANH SÁCH SẢN PHẨM</b>\n━━━━━━━━━━━━━━\nTổng acc còn: <b>${accs.length}</b>`,{parse_mode:'HTML'}); for(const a of accs){ const cap=`#${a.id} · <b>${escapeHtml(a.title)}</b>\n${escapeHtml(a.description||'')}\nGiá: <b>${vn(a.price)}đ</b>`; if(a.image_file_id) await bot.sendPhoto(chat,a.image_file_id,{caption:cap,parse_mode:'HTML'}); else await bot.sendMessage(chat,cap,{parse_mode:'HTML'}); } return; }
      ...
      if(data==='acc:top'){ const top=db.prepare('SELECT first_name,username,balance FROM accshop_users WHERE bot_id=? ORDER BY balance DESC LIMIT 10').all(row.id); return bot.sendMessage(chat, top.length ? '🏆 <b>TOP SỐ DƯ</b>\n━━━━━━━━━━━━━━\n'+top.map((u,i)=>`${i+1}. ${escapeHtml(u.first_name||u.username||'User')} — <b>${vn(u.balance)}đ</b>`).join('\n') : 'Chưa có dữ liệu top.', {parse_mode:'HTML'}); }
      if(data.startsWith('accadmin:')){ if(!isAccAdmin(cur,uid)) return; const a=data.split(':')[1]; if(a==='themacc'){ state.set(uid,'wait_acc'); return bot.sendMessage(chat,'📦 Gửi mô tả acc theo mẫu, có thể kèm ảnh:\nTên: ...\nGiá: 10000\nMô tả: ...\nAcc: user|pass'); } if(a==='thongbao') return bot.sendMessage(chat,'📢 Gửi theo mẫu:\n/thongbao nội_dung'); if(a==='bank') return bot.sendMessage(chat,'🏦 Gửi theo mẫu:\n/bank BANK|STK|CHỦ TK|NAP{random}'); if(a==='listadmin') return bot.sendMessage(chat,'Admin IDs: '+jsonArr(cur.admin_ids).join(', ')); }
