...
const CONFIG = {
  ...
  JWT_SECRET: "tgbot_platform_secret_key_2024_change_me",
...
db.exec(`
  CREATE TABLE IF NOT EXISTS bots (
    ...
    welcome_message TEXT DEFAULT 'Chào mừng bạn! 👋 Nhấn nút bên dưới để mở ứng dụng.',
...
function startBot(botConfig) {
  ...
  try {
    ...
    bot.onText(/\/start/, async (msg) => {
      ...
      // Gửi tin nhắn chào mừng + nút mở Mini App
...
app.post("/api/create-bot", async (req, res) => {
  ...
  if (api_key !== CONFIG.ADMIN_PASSWORD) {
...
app.delete("/api/delete-bot", (req, res) => {
  ...
  if (api_key !== CONFIG.ADMIN_PASSWORD) return res.status(401).json({ error: "api_key không hợp lệ" });
  const bot = db.prepare("SELECT * FROM bots WHERE token = ?").get(token);
  if (!bot) return res.status(404).json({ error: "Không tìm thấy bot" });
  stopBot(token);
...
app.listen(CONFIG.PORT, () => {
  ...
  console.log(`🔌 API Key cho web riêng: ${CONFIG.ADMIN_PASSWORD}\n`);
  loadAllBots();
});
