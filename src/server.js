...
async function startAccShopBot(row){
  ...
  const bot=new TelegramBot(row.token,{polling:{interval:700,autoStart:true,params:{timeout:20}}});
  ...
  function adminKeyboard(){ return {inline_keyboard:[
    [{text:'➕ Danh mục',callback_data:'accadmin:addcat'},{text:'📂 List danh mục',callback_data:'accadmin:listcat'}],
    ...
    [{text:'📢 Thông báo',callback_data:'accadmin:thongbao'},{text:'🗑️ Lệnh xoá',callback_data:'accadmin:deletehelp'}],
    [{text:'🛍️ Menu user',callback_data:'acc:start'}]
  ]}; }
  async function sendUserMenu(chat, cur){
    const note = cur.menu_note ? '\n\n📝 <b>Lưu ý:</b>\n' + escapeHtml(cur.menu_note) : '\n\n📝 <b>Lưu ý:</b> Bấm nút bên dưới để thao tác nhanh. Shop tự chuyển trang bằng nút, không spam lệnh.';
    return bot.sendMessage(chat,'🛍️ <b>DANH SÁCH SẢN PHẨM</b>\n━━━━━━━━━━━━━━━━━━━━\nChào mừng bạn đến với cửa hàng! Vui lòng chọn chức năng bên dưới:'+note,{parse_mode:'HTML',reply_markup:userKeyboard()});
  }
  async function sendAdminMenu(chat){
    return bot.sendMessage(chat,'✅ <b>QUẢN LÝ SHOP</b>\n━━━━━━━━━━━━━━━━━━━━\nChọn nút bên dưới để cấu hình nhanh. Các lệnh nâng cao chỉ dùng nội bộ trong khung chat này.\n\n<code>/danhmuc Tên danh mục</code>\n<code>/themacclist Danh mục|Giá</code> rồi xuống dòng list <code>user|pass</code>\n<code>/addacc Tên|Giá|Thông tin|Trạng thái|Danh mục</code> + gửi ảnh nếu muốn\n<code>/addzalopay Tên ví</code> + gửi ảnh QR hoặc caption ảnh\n<code>/addmomo Tên ví</code> + gửi ảnh QR hoặc caption ảnh\n<code>/xoaacc ID</code> · <code>/xoadanhmuc ID/tên</code> · <code>/xoakho</code>\n<code>/sethotro nội dung</code> hoặc gửi ảnh kèm caption\n<code>/thongbao nội_dung</code>',{parse_mode:'HTML',reply_markup:adminKeyboard()});
  }
  async function sendSupport(chat, cur){
    const txt = (cur.support_text||'').trim() || '👨‍💻 Liên hệ admin/shop để được hỗ trợ.';
  ...
  async function sendCats(chat){
    ...
    return bot.sendMessage(chat,'🛍️ <b>CHỌN DANH MỤC ACC</b>\n━━━━━━━━━━━━━━━━━━━━\nBấm vào danh mục để xem từng acc, có nút chuyển tiếp acc tiếp theo.',{parse_mode:'HTML',reply_markup:{inline_keyboard:keys}});
  }
  async function sendAccCard(chat, catId, idx=0){
    ...
    if(a.image_file_id) return bot.sendPhoto(chat,a.image_file_id,{caption:cap,parse_mode:'HTML',reply_markup:kb});
    return bot.sendMessage(chat,cap,{parse_mode:'HTML',reply_markup:kb});
  }
  ...
  bot.on('message',async msg=>{
    try{
      ...
      if(st==='wait_topup' && /^\d{4,}$/.test(text)){ const amount=Math.max(1000,Number(text)); const code=(cur.wallet_note_template||cur.bank_template||'NAP{random}').replace('{random}',crypto.randomBytes(3).toString('hex').toUpperCase()).replace('{uid}',String(uid)); db.prepare('INSERT INTO accshop_topups(bot_id,tg_user_id,amount,code) VALUES(?,?,?,?)').run(row.id,uid,amount,code); state.delete(uid); const hasWallet=(cur.wallet_qr_file_id||'').trim(); const qr=hasWallet?cur.wallet_qr_file_id:vietQr(cur,amount,code); const payName=hasWallet?((cur.wallet_type||'Ví').toUpperCase()):'VIETQR'; const cap=hasWallet ? `💳 <b>TẠO ĐƠN NẠP ${escapeHtml(payName)}</b>\n━━━━━━━━━━━━━━\nSố tiền: <b>${vn(amount)}đ</b>\nVí: <b>${escapeHtml(cur.wallet_name||cur.wallet_type||'')}</b>\nNội dung/Ghi chú: <code>${escapeHtml(code)}</code>\n\n⚠️ Quét QR và ghi đúng nội dung để shop duyệt nhanh.` : `💳 <b>TẠO ĐƠN NẠP VIETQR</b>\n━━━━━━━━━━━━━━\nSố tiền: <b>${vn(amount)}đ</b>\nBank: <b>${escapeHtml(cur.bank_name)}</b>\nSTK: <code>${escapeHtml(cur.bank_account)}</code>\nTên TK: <b>${escapeHtml(cur.bank_owner)}</b>\nNội dung: <code>${escapeHtml(code)}</code>`; if(qr) await bot.sendPhoto(chat,qr,{caption:cap,parse_mode:'HTML'}); else await bot.sendMessage(chat,cap+'\n⚠️ Shop chưa cấu hình phương thức nạp.',{parse_mode:'HTML'}); await notifyAdmins(`🔔 User <b>${escapeHtml(msg.from.first_name||msg.from.username||uid)}</b> vừa tạo đơn nạp <b>${vn(amount)}đ</b>\nNội dung: <code>${escapeHtml(code)}</code>`); return; }
      ...
      if(st==='wait_listacc' && isAccAdmin(cur,uid)){ const lines=(text||msg.caption||'').split(/\n+/).map(x=>x.trim()).filter(Boolean); let header=state.get(uid+':listHeader')||'Acc Game|0'; let [catName,priceRaw]=header.split('|').map(x=>x.trim()); const c=findCat(catName); const price=Number((priceRaw||'').replace(/\D/g,''))||0; let ok=0; for(const line of lines){ const parts=line.split(/\s+/).filter(Boolean); for(const sec of parts){ if(!sec.includes('|')) continue; db.prepare('INSERT INTO accshop_accounts(bot_id,category_id,title,description,secret,price,status) VALUES(?,?,?,?,?,?,?)').run(row.id,c.id,`Acc ${c.name}`,`Login: ${sec}`,sec,price,'on'); ok++; } } state.delete(uid); state.delete(uid+':listHeader'); await bot.sendMessage(chat,`✅ Đã thêm <b>${ok}</b> acc list vào danh mục <b>${escapeHtml(c.name)}</b>.\n📦 Kho còn: <b>${catStock(c.id)}</b>`,{parse_mode:'HTML'}); return; }
      ...
      if(['admin','themacc','addacc','themacclist','danhmuc','xoadanhmuc','listdanhmuc','xoaacc','xoakho','thongbao','bank','addzalopay','addmomo','listadmin','sethotro'].includes(cmd) && !isAccAdmin(cur,uid)){ return; }
      ...
      if(cmd==='themacclist'){ state.set(uid,'wait_listacc'); state.set(uid+':listHeader',arg||'Acc Game|0'); await bot.sendMessage(chat,'📚 Gửi list acc, mỗi dòng hoặc cách nhau bằng khoảng trắng dạng:\n<code>user|pass</code>\n<code>user2|pass2</code>\n\nHeader hiện tại: <b>'+escapeHtml(arg||'Acc Game|0')+'</b>\nMẫu lệnh: <code>/themacclist Acc Clone LV5|15000</code>',{parse_mode:'HTML'}); return; }
      ...
      if(cmd==='listadmin'){ await bot.sendMessage(chat,'Admin IDs: '+jsonArr(cur.admin_ids).join(', ')); return; }
      ...
      if(cmd==='lichsu'){ const rows=db.prepare('SELECT * FROM accshop_topups WHERE bot_id=? AND tg_user_id=? ORDER BY id DESC LIMIT 10').all(row.id,uid); await bot.sendMessage(chat, rows.length? rows.map(x=>`#${x.id} ${vn(x.amount)}đ · ${x.status} · ${x.code}`).join('\n'):'Chưa có lịch sử.',{reply_markup:userKeyboard()}); return; }
  ...
  bot.on('callback_query', async q=>{
    try{
      ...
      if(data==='acc:start') return sendUserMenu(chat,cur);
      if(data==='acc:cats'||data==='acc:kho') return sendCats(chat);
      if(data.startsWith('acc:cat:')){ const [, , catId, idx]=data.split(':'); return sendAccCard(chat,Number(catId),Number(idx)); }
      if(data.startsWith('acc:buy:')){ const id=Number(data.split(':')[2]); const a=db.prepare('SELECT * FROM accshop_accounts WHERE bot_id=? AND id=? AND sold=0').get(row.id,id); if(!a) return bot.sendMessage(chat,'❌ Acc này đã hết hoặc bị xoá.'); return bot.sendMessage(chat,`🛒 <b>XÁC NHẬN MUA</b>\n━━━━━━━━━━━━\n${escapeHtml(a.title)}\nGiá: <b>${vn(a.price)}đ</b>\n\nHiện bản này chỉ giữ hàng và báo admin. Admin có thể duyệt/giao thủ công.`,{parse_mode:'HTML',reply_markup:{inline_keyboard:[[{text:'👨‍💻 Liên hệ hỗ trợ',callback_data:'acc:hotro'},{text:'📂 Xem tiếp',callback_data:'acc:cats'}]]}}); }
      ...
      if(data==='acc:profile'){ const u=db.prepare('SELECT * FROM accshop_users WHERE bot_id=? AND tg_user_id=?').get(row.id,uid); return bot.sendMessage(chat,`💎 <b>THÔNG TIN TÀI KHOẢN CÁ NHÂN</b>\n━━━━━━━━━━━━━━━━━━━━\nID: <code>${uid}</code>\nUser: @${escapeHtml(q.from.username||'')}\nSố dư: <b>${vn(u?.balance||0)}đ</b>`,{parse_mode:'HTML',reply_markup:userKeyboard()}); }
      if(data==='acc:lichsu'){ const rows=db.prepare('SELECT * FROM accshop_topups WHERE bot_id=? AND tg_user_id=? ORDER BY id DESC LIMIT 10').all(row.id,uid); return bot.sendMessage(chat, rows.length? rows.map(x=>`#${x.id} ${vn(x.amount)}đ · ${x.status} · ${x.code}`).join('\n'):'Chưa có lịch sử.',{reply_markup:userKeyboard()}); }
      if(data==='acc:hotro') return sendSupport(chat,cur);
      if(data==='acc:top'){ const top=db.prepare('SELECT first_name,username,balance FROM accshop_users WHERE bot_id=? ORDER BY balance DESC LIMIT 10').all(row.id); return bot.sendMessage(chat, top.length ? '🏆 <b>TOP SỐ DƯ</b>\n━━━━━━━━━━━━━━\n'+top.map((u,i)=>`${i+1}. ${escapeHtml(u.first_name||u.username||'User')} — <b>${vn(u.balance)}đ</b>`).join('\n') : 'Chưa có dữ liệu top.', {parse_mode:'HTML',reply_markup:userKeyboard()}); }
      if(data.startsWith('accadmin:')){ if(!isAccAdmin(cur,uid)) return; const a=data.split(':')[1];
        ...
        if(a==='listcat') { const cats=db.prepare('SELECT * FROM accshop_categories WHERE bot_id=? ORDER BY id DESC').all(row.id); return bot.sendMessage(chat,cats.length?cats.map(c=>`#${c.id} ${c.name} — kho ${catStock(c.id)}`).join('\n'):'Chưa có danh mục.'); }
        ...
        if(a==='addlist'){ state.set(uid,'wait_listacc'); state.set(uid+':listHeader','Acc Game|0'); return bot.sendMessage(chat,'📚 Trước tiên có thể gửi lệnh: <code>/themacclist Tên danh mục|Giá</code>\nHoặc gửi luôn list user|pass, hệ thống lưu vào Acc Game.',{parse_mode:'HTML'}); }
        ...
        if(a==='deletehelp') return bot.sendMessage(chat,'🗑️ Lệnh xoá:\n/xoaacc ID\n/xoadanhmuc ID hoặc tên\n/xoakho');
      }
