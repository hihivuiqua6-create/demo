...
function serveHostedSite(row, req, res, next) {
  ...
  if (target.endsWith(".php")) {
    if (!PHP_BIN) {
      return res.status(500).type("html").send(`<!doctype html><meta charset="utf-8"><style>body{font-family:system-ui;background:#090a14;color:#fff;padding:24px}.box{max-width:720px;margin:auto;border:1px solid #333;border-radius:16px;padding:20px}</style><div class="box"><h2>Source PHP đã upload nhưng server chưa có PHP runtime</h2><p>Muốn chạy file PHP thật trên Render, deploy bằng Dockerfile có sẵn trong bản này hoặc cài PHP cho server. HTML/JS vẫn chạy bình thường.</p><p><b>File:</b> ${path.basename(target)}</p></div>`);
    }
    const env = {
      ...process.env,
