...
async function startAccShopBot(row){
  ...
  const bot=new TelegramBot(row.token,{polling:{interval:300,autoStart:true,params:{timeout:20,limit:50}}});
  ...
  bot.on('polling_error',e=>{ const msg=e.code||e.message; console.error(`[accshop #${row.id}]`,msg); if(/409|ETELEGRAM|ECONNRESET|ETIMEDOUT|EFATAL|socket hang up/i.test(String(msg))){ try{ bot.stopPolling({cancel:true}); }catch{}; setTimeout(()=>{ try{ bot.startPolling({restart:true}); }catch{} }, 1000); } });
