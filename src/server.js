...
CREATE TABLE IF NOT EXISTS shop_admin_keys (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  bot_id INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS shop_cmd_state (
  bot_id INTEGER NOT NULL,
  tg_user_id INTEGER NOT NULL,
  state TEXT,
...
safeAlter("ALTER TABLE bots ADD COLUMN sale_bot_enabled INTEGER DEFAULT 1");
safeAlter("ALTER TABLE bots ADD COLUMN admin_notify_chat_id INTEGER");
safeAlter("ALTER TABLE bots ADD COLUMN sale_bot_token TEXT");
safeAlter("ALTER TABLE bots ADD COLUMN sale_bot_username TEXT");
...
for (const [k, v] of [["bank_name", ""], ["bank_account", ""], ["bank_owner", ""], ["bank_qr_url", ""], ["topup_prefix", "NAP"], ["vietqr_bank_code", ""], ["vietqr_template", "compact2"]]) {
...
function logMsg(bot_id, tg_user_id, direction, text) {
  // Chống phình database khi bot đông user: chỉ lưu text ngắn, bỏ media rác và tự cắt còn 2k ký tự.
  const t = String(text || "").slice(0, 2000);
  if (!t || t === "[media]" || t === "[sticker]") return;
  try {
    db.prepare("INSERT INTO bot_messages(bot_id,tg_user_id,direction,text) VALUES(?,?,?,?)")
      .run(bot_id, tg_user_id, direction, t);
  } catch (e) { console.error("[logMsg]", e.message); }
}


// ---------- BOT BÁN ACC GAME (lệnh Telegram) ----------
function money(n) { return Number(n || 0).toLocaleString("vi-VN") + "đ"; }
function genShopAdminKey() { return "ADM-" + crypto.randomBytes(5).toString("hex").toUpperCase(); }
function getShopState(botId, userId) {
  return db.prepare("SELECT * FROM shop_cmd_state WHERE bot_id=? AND tg_user_id=?").get(botId, userId);
}
function setShopState(botId, userId, state, payload = {}) {
  db.prepare(`INSERT INTO shop_cmd_state(bot_id,tg_user_id,state,payload,updated_at) VALUES(?,?,?,?,CURRENT_TIMESTAMP)
              ON CONFLICT(bot_id,tg_user_id) DO UPDATE SET state=excluded.state,payload=excluded.payload,updated_at=CURRENT_TIMESTAMP`)
    .run(botId, userId, state, JSON.stringify(payload || {}));
}
function clearShopState(botId, userId) { db.prepare("DELETE FROM shop_cmd_state WHERE bot_id=? AND tg_user_id=?").run(botId, userId); }
function isShopAdmin(botId, userId) {
  return !!db.prepare("SELECT 1 FROM shop_admins WHERE bot_id=? AND tg_user_id=?").get(botId, userId);
}
function createVietQrUrl(amount, code) {
  const bankCode = getSetting("vietqr_bank_code", getSetting("bank_name", ""));
  const account = getSetting("bank_account", "");
  const template = getSetting("vietqr_template", "compact2");
  if (!bankCode || !account) return "";
  const params = new URLSearchParams({ amount: String(amount || 0), addInfo: String(code || ""), accountName: getSetting("bank_owner", "") });
  return `https://img.vietqr.io/image/${encodeURIComponent(bankCode)}-${encodeURIComponent(account)}-${encodeURIComponent(template)}.png?${params.toString()}`;
}
function userKeyboard(isAdmin = false) {
  const rows = [
    [{ text: "💳 Nạp tiền" }, { text: "📦 Xem kho acc" }],
    [{ text: "🧾 Lịch sử mua" }, { text: "ℹ️ Lệnh" }],
  ];
  if (isAdmin) rows.push([{ text: "🛡️ Bảng lệnh admin" }]);
  return { keyboard: rows, resize_keyboard: true };
}
function adminHelp() {
  return "🛡️ <b>BẢNG LỆNH ADMIN ẨN</b>\n━━━━━━━━━━━━━━━━━━\n" +
    "/themacc — thêm acc: bot sẽ hỏi mô tả/giá/tồn kho/ảnh\n" +
    "/thongbao nội_dung — gửi thông báo toàn bộ user bot\n" +
    "/keyadmin KEY — đăng nhập admin bằng key chủ cấp\n" +
    "/taokeyadmin ghi_chú — tạo key admin lẻ mới\n" +
    "/listkeyadmin — xem key admin\n" +
    "/bank — xem cấu hình bank/VietQR\n" +
    "/setbank BANKCODE|STK|CHU TK — đổi bank nhanh, ví dụ: /setbank MB|123|NGUYEN VAN A\n" +
    "/huy — hủy thao tác đang nhập";
}
function userHelp(isAdmin=false) {
  return "🤖 <b>BOT BÁN ACC GAME</b>\n━━━━━━━━━━━━━━━━━━\n" +
    "/start — bắt đầu\n/nap — tạo đơn nạp tiền VietQR\n/khoacc — xem kho acc còn lại\n/lichsu — xem lịch sử mua\n" +
    (isAdmin ? "\nDùng /admin để mở bảng lệnh admin ẩn." : "");
}
async function notifyAdmins(tg, botId, text, opts = {}) {
  const chats = new Set();
  const b = db.prepare("SELECT controller_tg_id, admin_notify_chat_id FROM bots WHERE id=?").get(botId) || {};
  if (b.controller_tg_id) chats.add(Number(b.controller_tg_id));
  if (b.admin_notify_chat_id) chats.add(Number(b.admin_notify_chat_id));
  for (const a of db.prepare("SELECT tg_user_id FROM shop_admins WHERE bot_id=?").all(botId)) chats.add(Number(a.tg_user_id));
  for (const chatId of chats) {
    try { await tg.sendMessage(chatId, text, { parse_mode: "HTML", ...opts }); } catch {}
  }
}
async function handleSaleBotMessage(tg, botId, msg) {
  if (!msg.from || msg.from.is_bot) return false;
  const text = String(msg.text || "").trim();
  const userId = msg.from.id;
  const admin = isShopAdmin(botId, userId) || !!(db.prepare("SELECT controller_tg_id FROM bots WHERE id=?").get(botId)?.controller_tg_id === userId);
  const isCmd = text.startsWith("/");
  const simple = text.toLowerCase();
  const send = (body, extra={}) => tg.sendMessage(msg.chat.id, body, { parse_mode: "HTML", reply_markup: userKeyboard(admin), ...extra }).catch(()=>{});

  if (text === "/huy") { clearShopState(botId, userId); await send("✅ Đã hủy thao tác."); return true; }
  const st = getShopState(botId, userId);
  if (st && !isCmd) {
    let payload = {}; try { payload = JSON.parse(st.payload || "{}"); } catch {}
    if (st.state === "await_topup_amount") {
      const amount = Math.floor(Number(text.replace(/[^0-9]/g, "")) || 0);
      if (amount < 10000) { await send("⚠️ Số tiền tối thiểu 10.000đ. Nhập lại số tiền hoặc /huy."); return true; }
      const code = randTransferCode();
      const qr = createVietQrUrl(amount, code);
      db.prepare("INSERT INTO shop_topups(bot_id,tg_user_id,username,amount,transfer_code) VALUES(?,?,?,?,?)")
        .run(botId, userId, msg.from.username || null, amount, code);
      clearShopState(botId, userId);
      const cap = `💳 <b>ĐƠN NẠP TIỀN</b>\n━━━━━━━━━━━━━━━━━━\nSố tiền: <b>${money(amount)}</b>\nNgân hàng: <b>${escapeHtml(getSetting("bank_name"))}</b>\nSTK: <code>${escapeHtml(getSetting("bank_account"))}</code>\nChủ TK: <b>${escapeHtml(getSetting("bank_owner"))}</b>\nNội dung: <code>${escapeHtml(code)}</code>\n\nQuét QR hoặc chuyển đúng nội dung trên.`;
      if (qr) await tg.sendPhoto(msg.chat.id, qr, { caption: cap, parse_mode: "HTML", reply_markup: userKeyboard(admin) }).catch(()=>send(cap));
      else await send(cap + "\n\n⚠️ Chưa cấu hình VietQR bank code/STK.");
      await notifyAdmins(tg, botId, `🔔 <b>CÓ ĐƠN NẠP MỚI</b>\nUser: ${escapeHtml(msg.from.username ? '@'+msg.from.username : msg.from.first_name || String(userId))} (<code>${userId}</code>)\nSố tiền: <b>${money(amount)}</b>\nNội dung: <code>${escapeHtml(code)}</code>`);
      return true;
    }
    if (st.state === "addacc_desc" && admin) {
      setShopState(botId, userId, "addacc_price", { description: text });
      await send("💰 Nhập giá acc, ví dụ: <code>50000</code>"); return true;
    }
    if (st.state === "addacc_price" && admin) {
      const price = Math.max(0, Math.floor(Number(text.replace(/[^0-9]/g, "")) || 0));
      setShopState(botId, userId, "addacc_stock", { ...payload, price });
      await send("📦 Nhập số lượng/tồn kho acc, ví dụ: <code>10</code>"); return true;
    }
    if (st.state === "addacc_stock" && admin) {
      const stock = Math.max(1, Math.floor(Number(text.replace(/[^0-9]/g, "")) || 1));
      setShopState(botId, userId, "addacc_photo", { ...payload, stock });
      await send("🖼️ Gửi ảnh acc/game ngay bây giờ. Nếu không cần ảnh, nhắn <code>boqua</code>."); return true;
    }
    if (st.state === "addacc_photo" && admin && simple === "boqua") {
      const desc = payload.description || "Acc game";
      db.prepare("INSERT INTO game_accounts(bot_id,title,description,price,stock,created_by_tg) VALUES(?,?,?,?,?,?)")
        .run(botId, desc.split(/\n/)[0].slice(0,80) || "Acc game", desc, payload.price || 0, payload.stock || 1, userId);
      clearShopState(botId, userId); await send("✅ Đã thêm acc vào kho."); return true;
    }
  }
  if (st && st.state === "addacc_photo" && admin && msg.photo?.length) {
    let payload = {}; try { payload = JSON.parse(st.payload || "{}"); } catch {}
    const fileId = msg.photo[msg.photo.length - 1].file_id;
    const desc = payload.description || "Acc game";
    db.prepare("INSERT INTO game_accounts(bot_id,title,description,image_file_id,price,stock,created_by_tg) VALUES(?,?,?,?,?,?,?)")
      .run(botId, desc.split(/\n/)[0].slice(0,80) || "Acc game", desc, fileId, payload.price || 0, payload.stock || 1, userId);
    clearShopState(botId, userId); await send("✅ Đã thêm acc + ảnh vào kho."); return true;
  }

  if (text === "/start" || simple === "ℹ️ lệnh") { await send(userHelp(admin)); return true; }
  if (text === "/admin" || simple === "🛡️ bảng lệnh admin") { if (admin) await send(adminHelp()); return true; }
  if (text === "/nap" || simple === "💳 nạp tiền") { setShopState(botId, userId, "await_topup_amount"); await send("💵 Nhập số tiền muốn nạp, ví dụ: <code>50000</code>"); return true; }
  if (text === "/khoacc" || simple === "📦 xem kho acc") {
    const rows = db.prepare("SELECT * FROM game_accounts WHERE bot_id=? AND active=1 ORDER BY id DESC LIMIT 30").all(botId);
    if (!rows.length) { await send("📦 Kho acc hiện đang trống."); return true; }
    const body = rows.map((a,i)=>`${i+1}. <b>${escapeHtml(a.title)}</b>\n   Giá: <b>${money(a.price)}</b> · Còn: <b>${Math.max(0, a.stock-a.sold)}</b>\n   ${escapeHtml(a.description||"").slice(0,260)}`).join("\n\n");
    await send("📦 <b>KHO ACC GAME</b>\n━━━━━━━━━━━━━━━━━━\n" + body);
    return true;
  }
  if (text === "/lichsu" || simple === "🧾 lịch sử mua") {
    const rows = db.prepare("SELECT * FROM shop_purchases WHERE bot_id=? AND tg_user_id=? ORDER BY id DESC LIMIT 20").all(botId, userId);
    const tops = db.prepare("SELECT * FROM shop_topups WHERE bot_id=? AND tg_user_id=? ORDER BY id DESC LIMIT 10").all(botId, userId);
    const a = rows.length ? rows.map((r,i)=>`${i+1}. ${escapeHtml(r.title||'Acc')} · ${money(r.amount)} · ${escapeHtml(r.status)}`).join("\n") : "Chưa có lịch sử mua.";
    const b = tops.length ? tops.map((r,i)=>`${i+1}. Nạp ${money(r.amount)} · ${escapeHtml(r.status)} · <code>${escapeHtml(r.transfer_code)}</code>`).join("\n") : "Chưa có lịch sử nạp.";
    await send(`🧾 <b>LỊCH SỬ</b>\n━━━━━━━━━━━━━━━━━━\n<b>Mua:</b>\n${a}\n\n<b>Nạp:</b>\n${b}`); return true;
  }
  if (/^\/keyadmin\b/i.test(text)) {
    const key = text.replace(/^\/keyadmin(?:@\w+)?\s*/i, "").trim().toUpperCase();
    if (!key) { await send("⚠️ Cú pháp: <code>/keyadmin KEY</code>"); return true; }
    const row = db.prepare("SELECT * FROM shop_admin_keys WHERE bot_id=? AND admin_key=? AND active=1").get(botId, key);
    if (!row) { await send("❌ Key admin sai hoặc đã tắt."); return true; }
    db.prepare("INSERT OR IGNORE INTO shop_admins(bot_id,tg_user_id,username,first_name,role) VALUES(?,?,?,?,?)")
      .run(botId, userId, msg.from.username || null, msg.from.first_name || null, "admin");
    db.prepare("UPDATE shop_admin_keys SET used_by_tg=?, used_at=CURRENT_TIMESTAMP WHERE id=?").run(userId, row.id);
    await send("✅ Đăng nhập admin thành công.\n\n" + adminHelp()); return true;
  }
  if (admin && /^\/themacc\b/i.test(text)) { setShopState(botId, userId, "addacc_desc"); await send("📝 Nhập mô tả acc/game. Dòng đầu sẽ làm tên sản phẩm."); return true; }
  if (admin && /^\/taokeyadmin\b/i.test(text)) {
    const note = text.replace(/^\/taokeyadmin(?:@\w+)?\s*/i, "").trim(); const k = genShopAdminKey();
    db.prepare("INSERT INTO shop_admin_keys(bot_id,admin_key,note) VALUES(?,?,?)").run(botId, k, note || null);
    await send(`✅ Key admin mới: <code>${k}</code>`); return true;
  }
  if (admin && /^\/listkeyadmin\b/i.test(text)) {
    const rows = db.prepare("SELECT * FROM shop_admin_keys WHERE bot_id=? ORDER BY id DESC LIMIT 30").all(botId);
    await send(rows.length ? rows.map((r,i)=>`${i+1}. <code>${escapeHtml(r.admin_key)}</code> · ${r.active?'ON':'OFF'} · ${escapeHtml(r.note||'')}`).join("\n") : "Chưa có key admin."); return true;
  }
  if (admin && /^\/thongbao\b/i.test(text)) {
    const body = text.replace(/^\/thongbao(?:@\w+)?\s*/i, "").trim();
    if (!body) { await send("⚠️ Cú pháp: <code>/thongbao nội dung</code>"); return true; }
    const users = db.prepare("SELECT tg_user_id, chat_id FROM bot_users WHERE bot_id=? ORDER BY last_seen DESC").all(botId);
    await send(`📢 Đang gửi thông báo tới ${users.length} user...`);
    let sent=0, failed=0;
    for (const u of users) {
      try { await tg.sendMessage(u.chat_id || u.tg_user_id, body, { parse_mode: "HTML" }); sent++; } catch { failed++; }
      if ((sent+failed)%20===0) await new Promise(r=>setTimeout(r,1000));
    }
    await send(`✅ Gửi xong. Thành công: ${sent}, lỗi: ${failed}`); return true;
  }
  if (admin && /^\/setbank\b/i.test(text)) {
    const raw = text.replace(/^\/setbank(?:@\w+)?\s*/i, ""); const [bank, acc, owner] = raw.split("|").map(x=>String(x||"").trim());
    if (!bank || !acc) { await send("⚠️ Cú pháp: <code>/setbank BANKCODE|STK|CHU TK</code>"); return true; }
    setSetting("bank_name", bank); setSetting("vietqr_bank_code", bank); setSetting("bank_account", acc); if (owner) setSetting("bank_owner", owner);
    await send("✅ Đã cập nhật bank/VietQR."); return true;
  }
  if (admin && /^\/bank\b/i.test(text)) { await send(`🏦 Bank: <b>${escapeHtml(getSetting("bank_name"))}</b>\nSTK: <code>${escapeHtml(getSetting("bank_account"))}</code>\nChủ TK: <b>${escapeHtml(getSetting("bank_owner"))}</b>\nVietQR bank_code: <b>${escapeHtml(getSetting("vietqr_bank_code"))}</b>`); return true; }
  return false;
}

// ---------- BOT REGISTRY ----------
// liveBots maps bot_id → { main: Mini App bot, autoreply: Auto Reply bot, sale: Command/Sale bot }
...
async function startBot(row) {
  ...
  const entry = { main: null, autoreply: null, sale: null };
  ...
  try {
    ...
    main.on("message", async (msg) => {
      ...
      // Nếu đã cấu hình sale_bot_token riêng thì bot mini-app chỉ lo /start + menu web, không xử lý lệnh bán acc.
      try {
        const curBot = db.prepare("SELECT sale_bot_enabled, sale_bot_token FROM bots WHERE id=?").get(fresh.id);
        if (curBot && curBot.sale_bot_enabled !== 0 && !curBot.sale_bot_token) await handleSaleBotMessage(main, fresh.id, msg);
      } catch (e) { console.error("[sale bot/main-fallback]", e.message); }
    });

    main.onText(/^\/start/, async (msg) => {
      const sale = db.prepare("SELECT sale_bot_enabled, sale_bot_token FROM bots WHERE id=?").get(fresh.id);
      if (sale && sale.sale_bot_enabled !== 0 && !sale.sale_bot_token) return;
      const u = msg.from || {};
  ...
  // ----- Sale/Command bot riêng biệt (bot lệnh bán acc/admin) -----
  const saleToken = fresh.sale_bot_token && fresh.sale_bot_token !== fresh.token && fresh.sale_bot_token !== fresh.autoreply_token ? fresh.sale_bot_token : null;
  if (fresh.sale_bot_enabled !== 0 && saleToken) {
    try {
      const saleBot = new TelegramBot(saleToken, { polling: true });
      try {
        const me = await saleBot.getMe();
        if (me?.username) db.prepare("UPDATE bots SET sale_bot_username=? WHERE id=?").run(me.username, fresh.id);
        // Chỉ public lệnh user; lệnh admin (/keyadmin, /themacc, /thongbao...) vẫn dùng được nhưng không hiện trong menu command.
        await saleBot.setMyCommands([
          { command: "start", description: "Bắt đầu" },
          { command: "nap", description: "Nạp tiền bằng VietQR" },
          { command: "khoacc", description: "Xem kho acc game" },
          { command: "lichsu", description: "Xem lịch sử mua/nạp" },
        ]);
      } catch (e) { console.error("[getMe/setCommands sale]", e.message); }
      saleBot.on("polling_error", async (e) => {
        const msg = e.code || e.message || "";
        console.error(`[bot ${fresh.id} sale] polling`, msg);
        if (/409|ETELEGRAM|ECONNRESET|ETIMEDOUT|EFATAL|socket hang up/i.test(String(msg))) {
          try { await saleBot.stopPolling({ cancel: true }); } catch {}
          setTimeout(() => { try { saleBot.startPolling({ restart: true }); } catch (er) { console.error("[restart sale polling]", er.message); } }, 4000);
        }
      });
      saleBot.on("message", async (msg) => {
        if (!msg.from || msg.from.is_bot) return;
        try {
          upsertUser(fresh.id, msg);
          // Log nhẹ: chỉ lưu text/media marker, tránh ghi quá nhiều làm DB phình nhanh.
          logMsg(fresh.id, msg.from.id, "in", msg.text || (msg.photo ? "[photo]" : msg.document ? "[doc]" : "[media]"));
        } catch {}
        try { await handleSaleBotMessage(saleBot, fresh.id, msg); }
        catch (e) { console.error("[sale bot separate]", e.message); }
      });
      entry.sale = saleBot;
      console.log(`✅ Bot #${fresh.id} SALE COMMAND (separate) started`);
    } catch (e) {
      console.error(`❌ Bot #${fresh.id} SALE COMMAND fail:`, e.message);
    }
  }
...
async function stopBot(id) {
  ...
  try { if (entry.sale) await entry.sale.stopPolling(); } catch {}
  liveBots.delete(id);
...
app.get("/api/bank", anyAuth, (_req, res) => {
  res.json({
    ...
    topup_prefix: getSetting("topup_prefix", "NAP"), vietqr_bank_code: getSetting("vietqr_bank_code"), vietqr_template: getSetting("vietqr_template", "compact2"),
  });
});
app.post("/api/admin/bank", adminOnly, (req, res) => {
  const { bank_name, bank_account, bank_owner, bank_qr_url, topup_prefix, vietqr_bank_code, vietqr_template } = req.body || {};
  ...
  if (vietqr_bank_code !== undefined) setSetting("vietqr_bank_code", vietqr_bank_code);
  if (vietqr_template !== undefined) setSetting("vietqr_template", vietqr_template);
  res.json({ ok: true });
});
...
app.post("/api/bots", anyAuth, async (req, res) => {
  ...
  const {
    ...
    sale_bot_token, sale_bot_enabled, admin_notify_chat_id, controller_tg_id, public_url_override, site_slug
  } = req.body || {};
  ...
  if (sale_bot_token && !/^\d+:[A-Za-z0-9_-]{30,}$/.test(sale_bot_token))
    return res.status(400).json({ error: "Bot lệnh bán acc token sai định dạng" });
  ...
  try {
    ...
    const info = db.prepare(
      `INSERT INTO bots(token,web_url,web_mode,welcome,button_label,
                        ...
                        autoreply_token,autoreply_count,sale_bot_token,sale_bot_enabled,admin_notify_chat_id,controller_tg_id,public_url_override)
       VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
    ).run(
      ...
      sale_bot_token || null,
      sale_bot_enabled === false ? 0 : 1,
      admin_notify_chat_id ? Number(admin_notify_chat_id) : null,
...
app.patch("/api/bots/:id", anyAuth, async (req, res) => {
  ...
  const {
    ...
    autoreply_token, autoreply_count, sale_bot_token, sale_bot_enabled, admin_notify_chat_id, controller_tg_id, public_url_override, site_slug,
  } = req.body || {};
  ...
  if (sale_bot_token !== undefined) {
    if (sale_bot_token && !/^\d+:[A-Za-z0-9_-]{30,}$/.test(sale_bot_token))
      return res.status(400).json({ error: "Bot lệnh bán acc token sai định dạng" });
    db.prepare("UPDATE bots SET sale_bot_token=? WHERE id=?").run(sale_bot_token || null, id);
    restartNeeded = true;
  }
  if (sale_bot_enabled !== undefined) {
    db.prepare("UPDATE bots SET sale_bot_enabled=? WHERE id=?").run(sale_bot_enabled ? 1 : 0, id);
    restartNeeded = true;
  }
  if (admin_notify_chat_id !== undefined) {
    db.prepare("UPDATE bots SET admin_notify_chat_id=? WHERE id=?").run(admin_notify_chat_id ? Number(admin_notify_chat_id) : null, id);
  }
...
// ---------- GAME ACCOUNT BOT API ----------
app.get("/api/bots/:id/game-accounts", anyAuth, (req,res)=>{
  const id=Number(req.params.id); if(!canTouchBot(req,id)) return res.status(403).json({error:"Không có quyền"});
  res.json(db.prepare("SELECT * FROM game_accounts WHERE bot_id=? ORDER BY id DESC LIMIT 500").all(id));
});
app.post("/api/bots/:id/game-accounts", anyAuth, (req,res)=>{
  const id=Number(req.params.id); if(!canTouchBot(req,id)) return res.status(403).json({error:"Không có quyền"});
  const {title,description,price,stock,active}=req.body||{}; if(!title) return res.status(400).json({error:"Thiếu title"});
  const info=db.prepare("INSERT INTO game_accounts(bot_id,title,description,price,stock,active) VALUES(?,?,?,?,?,?)")
    .run(id,String(title).slice(0,100),description||"",Math.max(0,Number(price)||0),Math.max(1,Number(stock)||1),active===false?0:1);
  res.json({ok:true,id:info.lastInsertRowid});
});
app.patch("/api/bots/:id/game-accounts/:accId", anyAuth, (req,res)=>{
  const id=Number(req.params.id), accId=Number(req.params.accId); if(!canTouchBot(req,id)) return res.status(403).json({error:"Không có quyền"});
  const old=db.prepare("SELECT * FROM game_accounts WHERE id=? AND bot_id=?").get(accId,id); if(!old) return res.status(404).json({error:"Không tìm thấy"});
  const next={...old,...req.body};
  db.prepare("UPDATE game_accounts SET title=?,description=?,price=?,stock=?,active=? WHERE id=? AND bot_id=?")
    .run(String(next.title||old.title).slice(0,100),next.description||"",Math.max(0,Number(next.price)||0),Math.max(0,Number(next.stock)||0),next.active?1:0,accId,id);
  res.json({ok:true});
});
app.post("/api/bots/:id/admin-keys", anyAuth, (req,res)=>{
  const id=Number(req.params.id); if(!canTouchBot(req,id)) return res.status(403).json({error:"Không có quyền"});
  const key=genShopAdminKey(); db.prepare("INSERT INTO shop_admin_keys(bot_id,admin_key,note) VALUES(?,?,?)").run(id,key,req.body?.note||null);
  res.json({ok:true,key});
});
app.get("/api/bots/:id/admin-keys", anyAuth, (req,res)=>{
  const id=Number(req.params.id); if(!canTouchBot(req,id)) return res.status(403).json({error:"Không có quyền"});
  res.json(db.prepare("SELECT id,admin_key,note,active,used_by_tg,used_at,created_at FROM shop_admin_keys WHERE bot_id=? ORDER BY id DESC LIMIT 100").all(id));
});
app.patch("/api/bots/:id/sale-settings", anyAuth, async (req,res)=>{
  const id=Number(req.params.id); if(!canTouchBot(req,id)) return res.status(403).json({error:"Không có quyền"});
  const {sale_bot_enabled, admin_notify_chat_id}=req.body||{};
  if(sale_bot_enabled!==undefined) db.prepare("UPDATE bots SET sale_bot_enabled=? WHERE id=?").run(sale_bot_enabled?1:0,id);
  if(admin_notify_chat_id!==undefined) db.prepare("UPDATE bots SET admin_notify_chat_id=? WHERE id=?").run(admin_notify_chat_id?Number(admin_notify_chat_id):null,id);
  await restartBot(id); res.json({ok:true});
});
