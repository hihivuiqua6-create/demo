...
async function startBot(row) {
  ...
  try {
    ...
    main.onText(/^\/start\b/, async (msg) => {
      const u = msg.from || {};
      const sk = `main:${fresh.id}:${u.id}:${msg.chat.id}`;
      if (cooldownLeft(mainStartCooldown, sk, 8000) > 0) return;
      mainStartCooldown.set(sk, Date.now());
...
const ACCSHOP_START_COOLDOWN_MS = 8 * 1000;
...
async function startAccShopBot(row){
  ...
  const bot=new TelegramBot(row.token,{polling:{interval:250,autoStart:true,params:{timeout:30}}});
  ...
  bot.on('polling_error',e=>{ const msg=e.code||e.message; console.error(`[accshop #${row.id}]`,msg); if(/409|ETELEGRAM|ECONNRESET|ETIMEDOUT|EFATAL/i.test(String(msg))){ try{ bot.stopPolling({cancel:true}); }catch{}; setTimeout(()=>{ try{ bot.startPolling({restart:true}); }catch{} }, 3000); } });
...
Chọn duyệt nếu đã nhận tiền.`,{reply_markup:{inline_keyboard:[[{text:'✅ Duyệt + cộng tiền',callback_data:`accpay:ok:${topupId}`},{text:'❌ Từ chối',callback_data:`accpay:no:${topupId}`}],[{text:'👤 Xem UID',callback_data:`accpay:user:${uid}`}]]}}); return; }
      ...
      if(cmd==='start'){ await tryDeleteIncoming(msg); const sk=`${row.id}:${uid}:${chat}`; if(cooldownLeft(accShopStartCooldown,sk,ACCSHOP_START_COOLDOWN_MS)>0) return; accShopStartCooldown.set(sk,Date.now()); await sendUserMenu(chat,cur); return; }
