...
const accShopBuyCooldown   = new Map(); // key -> last ts
...
async function startAccShopBot(row){
  ...
  const bot=new TelegramBot(row.token,{polling:{interval:50,autoStart:true,params:{timeout:20}}});
...
Ngày vào: ${escapeHtml(u.created_at||'')}`:'Không thấy user này.',{parse_mode:'HTML'}); return; }
  ...
  bot.on('callback_query', async q=>{
    try{
      ...
      if(data.startsWith('acc:buy:')){ const buyKey=`${row.id}:${uid}`; const waitMs=cooldownLeft(accShopBuyCooldown,buyKey,ACCSHOP_BUY_COOLDOWN_MS); if(waitMs>0) return renderPage(chat,`⏳ Bạn chỉ được tạo/mua <b>1 đơn acc mỗi 5 phút</b>.\nVui lòng thử lại sau <b>${Math.ceil(waitMs/60000)}</b> phút.`,{reply_markup:{inline_keyboard:[[{text:'📂 Xem kho',callback_data:'acc:cats'},{text:'🧾 Acc đã mua',callback_data:'acc:mybuys'}]]}},msg); const id=Number(data.split(':')[2]); const a=db.prepare('SELECT * FROM accshop_accounts WHERE bot_id=? AND id=? AND sold=0').get(row.id,id); if(!a) return renderPage(chat,'❌ Acc này đã hết hoặc bị xoá.',{reply_markup:{inline_keyboard:[[{text:'📂 Xem danh mục',callback_data:'acc:cats'}]]}},msg); const u=db.prepare('SELECT * FROM accshop_users WHERE bot_id=? AND tg_user_id=?').get(row.id,uid); const bal=Number(u?.balance||0); if(bal<Number(a.price||0)) return renderPage(chat,`❌ <b>KHÔNG ĐỦ SỐ DƯ</b>
━━━━━━━━━━━━
