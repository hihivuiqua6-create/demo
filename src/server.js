...
// v5.9 — Anti-crash: KHÔNG để 1 lỗi không bắt làm sập cả server.
// Trước đây các lỗi từ gramjs/telegram (FLOOD, NETWORK, AUTH_KEY_DUPLICATED, v.v.)
// thỉnh thoảng nổi lên process-level và làm Node thoát → toàn bộ bot/rải tắt.
process.on("uncaughtException", (err) => {
  try { console.error("[FATAL uncaughtException]", err && (err.stack || err.message) || err); } catch {}
});
process.on("unhandledRejection", (reason) => {
  try { console.error("[FATAL unhandledRejection]", reason && (reason.stack || reason.message) || reason); } catch {}
});
...
async function startAccountClient(row) {
  ...
  try {
    ...
    try { userbotCmd.attachCommandHandler(db, row.id, client); }
    catch (e) { console.error(`[acc #${row.id}] attach cmd:`, e.message); }
    console.log(`[acc #${row.id}] connected (${row.phone}) + userbot cmds`);
    return live;
...
setInterval(() => { tickAccountJobs().catch((e) => console.error("[tickAccountJobs]", e.message)); }, 100);
setInterval(() => { userbotCmd.tickRunner(db, liveAccounts).catch((e) => console.error("[tickRunner]", e.message)); }, 500);

// v5.9 — Watcher tự kết nối lại các acc bị rớt MTProto.
// Đảm bảo "bot rải off telegram vẫn cứ rải": ngay khi MTProto reconnect xong,
// runner sẽ rải tiếp mà không cần thao tác tay.
setInterval(async () => {
  try {
    const rows = db.prepare("SELECT * FROM user_accounts WHERE active=1").all();
    for (const r of rows) {
      const live = liveAccounts.get(r.id);
      if (!live) {
        startAccountClient(r).catch((e) => console.error(`[reconnect #${r.id}]`, e.message));
        continue;
      }
      // gramjs: client.connected là boolean. Nếu false → thử connect lại.
      if (live.client && live.client.connected === false) {
        try {
          await live.client.connect();
          console.log(`[reconnect #${r.id}] re-connected`);
        } catch (e) {
          // tách hẳn: drop client cũ rồi thử dựng lại từ session
          console.error(`[reconnect #${r.id}] fail:`, e.message);
          try { await live.client.disconnect(); } catch {}
          liveAccounts.delete(r.id);
          startAccountClient(r).catch(() => {});
        }
      }
    }
  } catch (e) {
    console.error("[acc watcher]", e.message);
  }
}, 10000);
...
app.listen(CONFIG.PORT, () => {
  console.log(`\n🚀 TGBot Platform v5.7 — http://localhost:${CONFIG.PORT}`);
