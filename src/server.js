...
async function startAccShopBot(row){
  ...
  bot.on('message',async msg=>{
    try{
      ...
      if(cmd==='start'){ await bot.sendMessage(chat,'🛍️ <b>DANH SÁCH LỆNH</b>\n━━━━━━━━━━━━━━\n/start - bắt đầu\n/nap - nạp tiền VietQR\n/kho - xem kho acc\n/profile - tài khoản\n/lichsu - lịch sử nạp/mua\n/hotro - hỗ trợ',{parse_mode:'HTML'}); return; }
      if(cmd==='keyadmin'||cmd==='keuadmin'){ if(!arg) return; if(String(arg).trim().toUpperCase()===String(cur.admin_key||'').toUpperCase()){ addAccAdmin(row.id,uid); await bot.sendMessage(chat,'✅ Đăng nhập admin thành công.\n\n🛡️ <b>BẢNG LỆNH ADMIN</b>\n/themacc - thêm acc bằng mô tả + ảnh\n/thongbao nội_dung - gửi toàn bộ user\n/bank BANK|STK|CHỦ TK|ND{random} - cấu hình bank\n/listadmin - xem admin\n/admin - hiện bảng lệnh',{parse_mode:'HTML'}); } else await bot.sendMessage(chat,'❌ Key admin sai.'); return; }
      if(['admin','themacc','thongbao','bank','listadmin'].includes(cmd) && !isAccAdmin(cur,uid)){ return; }
      if(cmd==='admin'){ await bot.sendMessage(chat,'🛡️ <b>LỆNH ADMIN ẨN</b>\n/themacc\n/thongbao nội_dung\n/bank BANK|STK|CHỦ TK|NAP{random}\n/listadmin',{parse_mode:'HTML'}); return; }
      ...
      if(cmd==='hotro'){ await bot.sendMessage(chat,'👨‍💻 Liên hệ admin/shop để được hỗ trợ.'); return; }
...
app.post('/api/accshop', adminOnly, async (req,res)=>{ const {token,owner_seller_id,bank_name,bank_account,bank_owner,bank_template}=req.body||{}; if(!/^\d+:[A-Za-z0-9_-]{30,}$/.test(token||'')) return res.status(400).json({error:'Token sai định dạng'}); try{ const key=accKey(); const info=db.prepare('INSERT INTO accshop_bots(token,owner_seller_id,admin_key,bank_name,bank_account,bank_owner,bank_template) VALUES(?,?,?,?,?,?,?)').run(token,owner_seller_id?Number(owner_seller_id):null,key,bank_name||'',bank_account||'',bank_owner||'',bank_template||'NAP{random}'); const row=db.prepare('SELECT * FROM accshop_bots WHERE id=?').get(info.lastInsertRowid); await startAccShopBot(row); res.json({id:row.id,admin_key:key}); }catch(e){res.status(400).json({error:e.message})} });
app.patch('/api/accshop/:id', anyAuth, async (req,res)=>{ const id=Number(req.params.id); if(!canTouchAccShop(req,id)) return res.status(403).json({error:'Không có quyền'}); const b=req.body||{}; if(req.user.role==='admin'){ for(const k of ['bank_name','bank_account','bank_owner','bank_template']) if(b[k]!==undefined) db.prepare(`UPDATE accshop_bots SET ${k}=? WHERE id=?`).run(String(b[k]||''),id); if(b.owner_seller_id!==undefined) db.prepare('UPDATE accshop_bots SET owner_seller_id=? WHERE id=?').run(b.owner_seller_id?Number(b.owner_seller_id):null,id); if(b.regen_key){ const k=accKey(); db.prepare('UPDATE accshop_bots SET admin_key=? WHERE id=?').run(k,id); } }
