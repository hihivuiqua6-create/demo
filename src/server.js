...
function serveHostedSite(row, req, res, next) {
  ...
  const root = path.join(CONFIG.SITES_DIR, row.web_zip_dir);
  const rel = decodeURIComponent((req.path || "/").replace(/^\/+/, ""));
  const pickIndex = () => ["index.html", "index.htm", "index.php"].map(f => path.join(root, f)).find(f => fs.existsSync(f));
  const target = rel ? path.join(root, rel) : pickIndex();
  if (target && target.startsWith(root) && fs.existsSync(target) && fs.statSync(target).isFile() && target.endsWith(".php")) {
    // Nếu server có PHP, chạy PHP để các source shop PHP như lucvi1k_shop_v29 hoạt động.
    try {
      const php = spawn("php", [target], { cwd: root, env: { ...process.env, REQUEST_METHOD: req.method, QUERY_STRING: req.url.split("?")[1] || "" } });
  ...
  express.static(root, {
    ...
    setHeaders: (res, filePath) => { if (filePath.endsWith(".php")) res.setHeader("Content-Type", "text/html; charset=utf-8"); },
  })(req, res, next);
}
...
app.post("/api/accounts/login/start", anyAuth, async (req, res) => {
  ...
  try {
    ...
    const sent = await withTimeout(client.sendCode(
      { apiId: Number(api_id), apiHash: String(api_hash).trim() },
      phoneClean
    ), 30000, "Gửi mã Telegram quá lâu / bị treo. Thử restart server hoặc tạo api_id/api_hash mới.");
    ...
    res.status(400).json({ error: e.message || "Không gửi được mã Telegram" });
  }
});
