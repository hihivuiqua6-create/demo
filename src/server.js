...
setInterval(async () => {
  ...
  for (const r of rows) {
    ...
    const now = Date.now();
    const wait = Math.max(10, Number(r.delay_seconds) || 300) * 1000;
    ...
    (async () => {
      try {
        ...
        const listDelay = Math.max(1, Number(r.list_delay_seconds) || 30) * 1000;
...
app.post("/api/autorai", anyAuth, async (req, res) => {
  ...
  try {
    const info = db.prepare(
      ...
      Math.max(10, Number(delay_seconds) || 300),
      Math.max(1, Number(list_delay_seconds) || 30),
      owner);
...
app.patch("/api/autorai/:id", anyAuth, async (req, res) => {
  ...
  if (delay_seconds !== undefined)
    db.prepare("UPDATE autorai_bots SET delay_seconds=? WHERE id=?").run(Math.max(10, Number(delay_seconds) || 300), id);
  if (list_delay_seconds !== undefined)
    db.prepare("UPDATE autorai_bots SET list_delay_seconds=? WHERE id=?").run(Math.max(1, Number(list_delay_seconds) || 30), id);
