...
// Render-safe Telegram polling options: nhẹ CPU/RAM, không spam request Telegram.
// Sửa lỗi v20: TG_POLLING_LIGHT bị thiếu nên mở/sửa bot là crash server và bot không chạy.
const TG_POLLING_LIGHT = {
  polling: {
    autoStart: true,
    interval: Number(process.env.TG_POLLING_INTERVAL || 1500),
    params: { timeout: Number(process.env.TG_POLLING_TIMEOUT || 25) }
  },
  request: {
    timeout: Number(process.env.TG_REQUEST_TIMEOUT || 30000),
    forever: true
  }
};
...
async function startBot(row) {
  ...
  // FIX v16: nếu token này đang dùng cho Bot Bán Acc thì KHÔNG chạy thêm main polling.
  // Chạy 2 polling cùng 1 token sẽ gây 409 Conflict -> /start lúc được lúc không, delay hoặc không trả lời.
  try {
    const accSameToken = db.prepare("SELECT id FROM accshop_bots WHERE token=? AND active=1 LIMIT 1").get(fresh.token);
    if (accSameToken) {
      console.log(`⏭️ Bot #${fresh.id} MAIN skipped: token đang chạy ở Bot Bán Acc #${accSameToken.id}`);
  ...
  try {
    const main = new TelegramBot(fresh.token, TG_POLLING_LIGHT);
    ...
    main.on("polling_error", async (e) => {
      ...
      if (/409/i.test(String(msg))) { try { await main.stopPolling({ cancel: true }); } catch {}; return; }
      if (/ETELEGRAM|ECONNRESET|ETIMEDOUT|EFATAL|socket hang up/i.test(String(msg))) {
        try { await main.stopPolling({ cancel: true }); } catch {}
        setTimeout(() => { try { main.startPolling({ restart: true }); } catch (er) { console.error("[restart polling]", er.message); } }, 10000);
      }
    });
    ...
    main.onText(/^\/start\b/, async (msg) => {
      const u = msg.from || {};
      // FIX v16: không cooldown /start nữa để bấm là bot trả lời ngay.
      // Vẫn tự xoá tin /start của user nếu bot có quyền.
  ...
  const arBot = arToken ? new TelegramBot(arToken, TG_POLLING_LIGHT) : entry.main;
  ...
  if (arBot) {
    if (arToken) {
      ...
      arBot.on("polling_error", async (e) => {
        ...
        if (/409/i.test(String(msg))) { try { await arBot.stopPolling({ cancel: true }); } catch {}; return; }
        if (/ETELEGRAM|ECONNRESET|ETIMEDOUT|EFATAL|socket hang up/i.test(String(msg))) {
          try { await arBot.stopPolling({ cancel: true }); } catch {}
          setTimeout(() => { try { arBot.startPolling({ restart: true }); } catch (er) { console.error("[restart ar polling]", er.message); } }, 10000);
        }
      });
    }
...
async function startAutoraiBot(row) {
  ...
  try {
    const bot = new TelegramBot(row.token, TG_POLLING_LIGHT);
    try {
...
}, Number(process.env.AUTORAI_TICK_MS || 1500));
...
setInterval(() => { tickAccountJobs().catch(() => {}); }, Number(process.env.ACCOUNT_JOB_TICK_MS || 3000));
setInterval(() => { userbotCmd.tickRunner(db, liveAccounts, startAccountClient).catch(() => {}); }, Number(process.env.USERBOT_CMD_TICK_MS || 3000));
...
// ===== Anti-spam & order limits (v15) =====
const accShopStartCooldown = new Map(); // key -> last ts
const accShopTopupCooldown = new Map(); // key -> last topup order ts (fallback RAM)
const mainStartCooldown    = new Map(); // key -> last ts
const ACCSHOP_START_COOLDOWN_MS = 0; // FIX v16: /start không bị delay/cooldown
const ACCSHOP_TOPUP_COOLDOWN_MS = 5 * 60 * 1000; // chỉ áp dụng tạo đơn nạp, KHÔNG áp dụng mua acc
const ACCSHOP_TOPUP_MIN = 1000;            // 1k
const ACCSHOP_TOPUP_MAX = 5_000_000_000_000; // 5,000,000,000k
function cooldownLeft(map,key,ms){ const t=Number(map.get(key)||0); const left=ms-(Date.now()-t); return left>0?left:0; }
function accTopupLockLeft(botId, uid){
  try{
    const p=db.prepare("SELECT id,created_at FROM accshop_topups WHERE bot_id=? AND tg_user_id=? AND status='pending' ORDER BY id DESC LIMIT 1").get(botId,uid);
    if(!p) return 0;
    const created = new Date(String(p.created_at).replace(' ','T') + 'Z').getTime();
    const age=Date.now()-(Number.isFinite(created)?created:Date.now());
    return Math.max(0, ACCSHOP_TOPUP_COOLDOWN_MS-age);
  }catch{ return cooldownLeft(accShopTopupCooldown,`${botId}:${uid}`,ACCSHOP_TOPUP_COOLDOWN_MS); }
}
...
async function startAccShopBot(row){
  ...
  // v20: chỉ cho 1 bot bán acc chạy cho mỗi token, tránh 409 Conflict spam làm sập Render.
  for(const [otherId, live] of liveAccShop){
    try{ const other=db.prepare('SELECT token FROM accshop_bots WHERE id=?').get(otherId); if(other && other.token===row.token && otherId!==row.id){ await stopAccShopBot(otherId); } }catch{}
  }
  // FIX v16: chống trùng token giữa Bot thường và Bot Bán Acc.
  // Nếu trùng mà cả 2 cùng polling, Telegram trả 409 và bot sẽ lúc trả lời lúc không.
  try{
    const normalBots=db.prepare('SELECT id FROM bots WHERE token=? AND active=1').all(row.token);
    for(const nb of normalBots){ if(liveBots.has(nb.id)) await stopBot(nb.id); }
  }catch{}
  const bot=new TelegramBot(row.token,TG_POLLING_LIGHT);
  const state=new Map(); liveAccShop.set(row.id,{bot,state});
  async function tryDeleteIncoming(msg){ try{ if(msg?.chat?.id && msg?.message_id) await bot.deleteMessage(msg.chat.id,msg.message_id); }catch{} }
  ...
  bot.on('polling_error',e=>{
    ...
    console.error(`[accshop #${row.id}]`,msg);
    // 409 nghĩa là token đang bị chạy ở nơi khác / instance khác. Không restart liên tục vì sẽ quá tải và sập Render.
    if(/409/i.test(msg)){ try{ bot.stopPolling({cancel:true}); }catch{}; return; }
    if(/ECONNRESET|ETIMEDOUT|EFATAL|socket hang up|ETELEGRAM/i.test(msg)){
      try{ bot.stopPolling({cancel:true}); }catch{}
      setTimeout(()=>{ try{ bot.startPolling({restart:true}); }catch{} }, 10000);
    }
  });
  ...
  function fmtBuyRows(rows){
    return rows.length ? rows.map((x,i)=>{
      ...
      return `${i+1}. ${escapeHtml(x.title||'Acc')} · <b>${vn(x.price)}đ</b>\n   📂 Loại/ID: <code>${x.account_id||''}</code> · ${escapeHtml(x.created_at||'')}${secret ? `\n   🔐 TK/MK: <code>${escapeHtml(secret)}</code>` : ''}`;
  ...
  function userKeyboard(){ return {inline_keyboard:[
    ...
    [{text:'💎 Cá Nhân',callback_data:'acc:profile'},{text:'🧾 Acc Đã Mua',callback_data:'acc:mybuys'}],
    [{text:'🏆 Top Nạp',callback_data:'acc:top'},{text:'🧾 Lịch Sử',callback_data:'acc:lichsu'}],
    [{text:'👨‍💻 Hỗ Trợ',callback_data:'acc:hotro'}]
  ]}; }
  ...
  function parseAccBody(body){
    ...
    const title=a[0]||'Acc game';
    const price=Number((a[1]||'').replace(/\D/g,''))||0;
    const desc=a[2]||'';
    const status=a[3]||'on';
    const cat=a[4]||'';
    // Mẫu mới: Tên acc|Giá|Thông tin|Trạng thái|Danh mục|user:pass
    // Trường cuối là dữ liệu giao cho khách sau khi mua. Nếu chưa nhập thì dùng phần Thông tin để tránh mất dữ liệu cũ.
    const secret=(a.slice(5).join('|').trim()) || desc;
    return {title,price,desc,status,cat,secret};
  }
  ...
  bot.on('message',async msg=>{
    try{
      ...
      if(st==='wait_topup' && /^\d+$/.test(text)){ await tryDeleteIncoming(msg); const rawAmount=Number(text); if(rawAmount<ACCSHOP_TOPUP_MIN||rawAmount>ACCSHOP_TOPUP_MAX){ return bot.sendMessage(chat,`⚠️ Số tiền nạp phải từ <b>${vn(ACCSHOP_TOPUP_MIN)}đ</b> đến <b>${vn(ACCSHOP_TOPUP_MAX)}đ</b>.`,{parse_mode:'HTML'}); } const lockLeft=accTopupLockLeft(row.id,uid); if(lockLeft>0){ return bot.sendMessage(chat,`⏳ Bạn còn đơn nạp đang chờ duyệt. Nếu chưa được duyệt/từ chối thì sau <b>${Math.ceil(lockLeft/60000)}</b> phút mới tạo đơn nạp tiếp.\n\n✅ Mua acc không bị giới hạn, chỉ giới hạn tạo đơn nạp.`,{parse_mode:'HTML'}); } const amount=rawAmount; const code=(cur.wallet_note_template||cur.bank_template||'NAP{random}').replace('{random}',crypto.randomBytes(3).toString('hex').toUpperCase()).replace('{uid}',String(uid)); const ins=db.prepare('INSERT INTO accshop_topups(bot_id,tg_user_id,amount,code,status) VALUES(?,?,?,?,?)').run(row.id,uid,amount,code,'pending'); accShopTopupCooldown.set(`${row.id}:${uid}`,Date.now()); const topupId=ins.lastInsertRowid; logAct(uid,'topup_create',`Tạo đơn nạp #${topupId} ${amount}đ`); state.delete(uid); const hasWallet=(cur.wallet_qr_file_id||'').trim(); const qr=hasWallet?cur.wallet_qr_file_id:vietQr(cur,amount,code); const payName=hasWallet?((cur.wallet_type||'Ví').toUpperCase()):'VIETQR'; const cap=hasWallet ? `💳 <b>TẠO ĐƠN NẠP ${escapeHtml(payName)}</b>
━━━━━━━━━━━━━━
...
Chọn duyệt nếu đã nhận tiền.`,{reply_markup:{inline_keyboard:[[{text:'✅ Duyệt + cộng tiền',callback_data:`accpay:ok:${topupId}`},{text:'❌ Từ chối',callback_data:`accpay:no:${topupId}`}],[{text:'👤 Xem UID',callback_data:`accpay:user:${uid}`}]]}}); return; }
      ...
      if(st==='wait_addacc' && isAccAdmin(cur,uid)){ const body=text||msg.caption||''; const p=parseAccBody(body); const c=findCat(p.cat); const img=msg.photo?.length?msg.photo[msg.photo.length-1].file_id:null; db.prepare('INSERT INTO accshop_accounts(bot_id,category_id,title,description,image_file_id,secret,price,status) VALUES(?,?,?,?,?,?,?,?)').run(row.id,c.id,p.title,p.desc,img,p.secret,p.price,p.status); state.delete(uid); await bot.sendMessage(chat,`✅ Đã thêm 1 acc vào <b>${escapeHtml(c.name)}</b>.`,{parse_mode:'HTML'}); return; }
      ...
      if(cmd==='start'){ await tryDeleteIncoming(msg); await sendUserMenu(chat,cur); return; }
      ...
      if(cmd==='addacc'||cmd==='themacc'){ if(arg){ const p=parseAccBody(arg); const c=findCat(p.cat); db.prepare('INSERT INTO accshop_accounts(bot_id,category_id,title,description,secret,price,status) VALUES(?,?,?,?,?,?,?)').run(row.id,c.id,p.title,p.desc,p.secret,p.price,p.status); await bot.sendMessage(chat,`✅ Đã thêm 1 acc vào ${c.name}.`); } else { state.set(uid,'wait_addacc'); await bot.sendMessage(chat,'📦 Gửi acc theo mẫu, có thể gửi kèm ảnh:\n<code>Tên acc|Giá|Thông tin mô tả|Trạng thái|Danh mục|user:pass</code>\nVD: Acc clone LV5|15000|Pet đẹp, rank cao|on|Acc Clone LV5|user1:pass1',{parse_mode:'HTML'}); } return; }
...
Đã giao tự động.`); return; }
      ...
      if(data==='acc:mybuys'){ const buys=db.prepare('SELECT * FROM accshop_purchases WHERE bot_id=? AND tg_user_id=? ORDER BY id DESC LIMIT 20').all(row.id,uid); if(!buys.length) return renderPage(chat,'🧾 <b>ACC ĐÃ MUA</b>\n━━━━━━━━━━━━\nBạn chưa mua acc nào.',{reply_markup:userKeyboard()},msg); const kb=buys.map(b=>[{text:`#${b.id} · ${(b.title||'Acc').slice(0,28)} · ${vn(b.price)}đ`,callback_data:`acc:mybuyview:${b.id}`}]); kb.push([{text:'⬅️ Menu',callback_data:'acc:start'}]); return renderPage(chat,`🧾 <b>ACC ĐÃ MUA</b>\n━━━━━━━━━━━━\nTổng: <b>${buys.length}</b> đơn (20 đơn gần nhất).\nBấm vào đơn để xem nội dung acc.`,{reply_markup:{inline_keyboard:kb}},msg); }
      if(data.startsWith('acc:mybuyview:')){ const pid=Number(data.split(':')[2]); const p=db.prepare('SELECT * FROM accshop_purchases WHERE bot_id=? AND tg_user_id=? AND id=?').get(row.id,uid,pid); if(!p) return renderPage(chat,'❌ Không thấy đơn này hoặc không thuộc về bạn.',{reply_markup:{inline_keyboard:[[{text:'⬅️ Quay lại',callback_data:'acc:mybuys'}]]}},msg); const secret=p.secret||'(không có dữ liệu)'; return renderPage(chat,`🧾 <b>CHI TIẾT ĐƠN #${p.id}</b>\n━━━━━━━━━━━━\nSản phẩm: <b>${escapeHtml(p.title||'Acc')}</b>\nGiá: <b>${vn(p.price)}đ</b>\nNgày mua: ${escapeHtml(p.created_at||'')}\n\n🔐 <b>NỘI DUNG ACC</b>\n<code>${escapeHtml(secret)}</code>`,{reply_markup:{inline_keyboard:[[{text:'⬅️ Acc đã mua',callback_data:'acc:mybuys'},{text:'🏠 Menu',callback_data:'acc:start'}]]}},msg); }
      ...
      if(data.startsWith('accadmin:')){ if(!isAccAdmin(cur,uid)) return; const a=data.split(':')[1];
        ...
        if(a==='addacc'){ state.set(uid,'wait_addacc'); return bot.sendMessage(chat,'📦 Gửi mẫu: <code>Tên acc|Giá|Thông tin|Trạng thái|Danh mục|user:pass</code>\nPhần <b>user:pass</b> sẽ hiện cho khách sau khi mua và lưu trong lịch sử.',{parse_mode:'HTML'}); }
...
app.get('/api/accshop/:id/user/:uid', anyAuth, (req,res)=>{ const id=Number(req.params.id), uid=Number(req.params.uid); if(!canTouchAccShop(req,id)) return res.status(403).json({error:'Không có quyền'}); const user=db.prepare('SELECT * FROM accshop_users WHERE bot_id=? AND tg_user_id=?').get(id,uid); if(!user) return res.status(404).json({error:'Không thấy user'}); const purchases=db.prepare('SELECT * FROM accshop_purchases WHERE bot_id=? AND tg_user_id=? ORDER BY id DESC LIMIT 50').all(id,uid); const topups=db.prepare('SELECT * FROM accshop_topups WHERE bot_id=? AND tg_user_id=? ORDER BY id DESC LIMIT 50').all(id,uid); const logs=db.prepare('SELECT * FROM accshop_activity WHERE bot_id=? AND tg_user_id=? ORDER BY id DESC LIMIT 80').all(id,uid); res.json({user,purchases,topups,logs}); });


// ---------- ACCSHOP BACKUP / IMPORT (admin) ----------
const ACCSHOP_BACKUP_TABLES = ['accshop_bots','accshop_users','accshop_categories','accshop_accounts','accshop_topups','accshop_purchases','accshop_activity'];
function tableCols(t){ return db.prepare(`PRAGMA table_info(${t})`).all().map(x=>x.name); }
function rowsOf(t){ return db.prepare(`SELECT * FROM ${t} ORDER BY id ASC`).all(); }
app.get('/api/accshop/backup', adminOnly, (req,res)=>{
  const data={ version:'accshop-backup-v17', exported_at:new Date().toISOString(), tables:{} };
  for(const t of ACCSHOP_BACKUP_TABLES) data.tables[t]=rowsOf(t);
  res.setHeader('Content-Disposition', `attachment; filename="accshop-backup-${Date.now()}.json"`);
  res.json(data);
});
app.post('/api/accshop/import', adminOnly, async (req,res)=>{
  const payload=req.body||{};
  const tables=payload.tables||payload;
  if(!tables || typeof tables!=='object') return res.status(400).json({error:'File backup không hợp lệ'});
  try{
    for(const [id] of liveAccShop) await stopAccShopBot(id);
    const tx=db.transaction(()=>{
      for(const t of [...ACCSHOP_BACKUP_TABLES].reverse()) db.prepare(`DELETE FROM ${t}`).run();
      for(const t of ACCSHOP_BACKUP_TABLES){
        const list=Array.isArray(tables[t])?tables[t]:[];
        const cols=tableCols(t);
        for(const row of list){
          const use=cols.filter(c=>Object.prototype.hasOwnProperty.call(row,c));
          if(!use.length) continue;
          const sql=`INSERT OR REPLACE INTO ${t} (${use.join(',')}) VALUES (${use.map(c=>'@'+c).join(',')})`;
    ...
    tx();
    let restarted=0;
    for(const r of db.prepare('SELECT * FROM accshop_bots WHERE active=1').all()){ await startAccShopBot(r); restarted++; }
    res.json({ok:true,restarted});
  }catch(e){ res.status(400).json({error:e.message||'Import thất bại'}); }
});



// ---------- FULL WEB BACKUP / IMPORT (admin) ----------
const FULL_BACKUP_TABLES = [
  'bots','bot_users','bot_messages','bot_autoreply_exceptions','broadcasts','sellers','topups','settings',
  'autorai_bots','seller_managed_bots','user_accounts','account_jobs',
  'accshop_bots','accshop_users','accshop_categories','accshop_accounts','accshop_topups','accshop_purchases','accshop_activity'
];
function rowsOfAny(t){
  const cols = tableCols(t);
  const order = cols.includes('id') ? ' ORDER BY id ASC' : '';
  return db.prepare(`SELECT * FROM ${t}${order}`).all();
}
app.get('/api/admin/backup-all', adminOnly, (req,res)=>{
  const data={ version:'full-web-backup-v18', exported_at:new Date().toISOString(), tables:{} };
  for(const t of FULL_BACKUP_TABLES) data.tables[t]=rowsOfAny(t);
  res.setHeader('Content-Disposition', `attachment; filename="full-web-backup-${Date.now()}.json"`);
  res.json(data);
});
app.post('/api/admin/import-all', adminOnly, async (req,res)=>{
  const payload=req.body||{};
  const tables=payload.tables||payload;
  if(!tables || typeof tables!=='object') return res.status(400).json({error:'File backup không hợp lệ'});
  try{
    for(const [id] of liveBots) await stopBot(id);
    for(const [id] of liveAutorai) await stopAutoraiBot(id);
    for(const [id] of liveAccShop) await stopAccShopBot(id);
    const tx=db.transaction(()=>{
      for(const t of [...FULL_BACKUP_TABLES].reverse()) db.prepare(`DELETE FROM ${t}`).run();
    ...
    tx();
    let restarted=0;
    for(const r of db.prepare('SELECT * FROM bots WHERE active=1').all()){ await startBot(r); restarted++; }
    for(const r of db.prepare('SELECT * FROM autorai_bots WHERE active=1').all()){ await startAutoraiBot(r); restarted++; }
    for(const r of db.prepare('SELECT * FROM accshop_bots WHERE active=1').all()){ await startAccShopBot(r); restarted++; }
    res.json({ok:true,restarted});
  }catch(e){ res.status(400).json({error:e.message||'Import toàn web thất bại'}); }
});
