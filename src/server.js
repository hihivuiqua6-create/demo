...
app.post("/api/account-jobs", anyAuth, (req, res) => {
  const { account_id, name, contents, channels, delay_seconds, list_delay_seconds, mentions, mention_mode, mentions_per_send } = req.body || {};
  ...
  const info = db.prepare(
    "INSERT INTO account_jobs(account_id,name,contents,channels,delay_seconds,list_delay_seconds,mentions,mention_mode,mentions_per_send) VALUES(?,?,?,?,?,?,?,?,?)"
  ).run(
    Number(account_id),
    String(name || "Job"),
...
app.patch("/api/account-jobs/:id", anyAuth, (req, res) => {
  ...
  const { name, contents, channels, delay_seconds, list_delay_seconds, running, mentions, mention_mode, mentions_per_send } = req.body || {};
    ...
    (async () => {
      try {
        ...
        for (const ch of channels) {
          ...
          if (!target) continue;
          try {
            const entity = await live.client.getEntity(/^-?\d+$/.test(target) ? Number(target) : target);
            await live.client.sendMessage(entity, { message: content, parseMode: "html" });
          } catch (e) {
            console.error(`[acc job #${r.id}] ${target}:`, e.message);
          }
