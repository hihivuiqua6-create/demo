...
async function startAccShopBot(row){
  ...
  async function sendUserMenu(chat, cur, srcMsg=null){
    const note = cur.menu_note ? '\n\n📝 <b>Lưu ý:</b>\n' + escapeHtml(cur.menu_note) : '\n\n📝 <b>Lưu ý:</b> Chọn nút bên dưới để chuyển trang nhanh. Bot chỉ gửi tin mới khi cần nhập thông tin như nạp tiền, thêm acc hoặc gửi ảnh.';
...
Chọn duyệt nếu đã nhận tiền.`,{reply_markup:{inline_keyboard:[[{text:'✅ Duyệt + cộng tiền',callback_data:`accpay:ok:${topupId}`},{text:'❌ Từ chối',callback_data:`accpay:no:${topupId}`}],[{text:'👤 Xem UID',callback_data:`accpay:user:${uid}`}]]}}); return; }
      ...
      if(['admin','themacc','addacc','themacclist','danhmuc','xoadanhmuc','listdanhmuc','xoaacc','xoakho','thongbao','bank','addzalopay','addmomo','listadmin','sethotro','congtien','trutien','duyettien','huytien'].includes(cmd) && !isAccAdmin(cur,uid)){ return; }
  ...
  bot.on('callback_query', async q=>{
    try{
      ...
      if(data.startsWith('acc:buy:')){ const id=Number(data.split(':')[2]); const a=db.prepare('SELECT * FROM accshop_accounts WHERE bot_id=? AND id=? AND sold=0').get(row.id,id); if(!a) return bot.sendMessage(chat,'❌ Acc này đã hết hoặc bị xoá.'); return renderPage(chat,`🛒 <b>XÁC NHẬN MUA</b>\n━━━━━━━━━━━━\n${escapeHtml(a.title)}\nGiá: <b>${vn(a.price)}đ</b>\n\nHiện bản này chỉ giữ hàng và báo admin. Admin có thể duyệt/giao thủ công.`,{reply_markup:{inline_keyboard:[[{text:'👨‍💻 Liên hệ hỗ trợ',callback_data:'acc:hotro'},{text:'📂 Xem tiếp',callback_data:'acc:cats'}]]}},msg); }
