...
// Render free dễ sập nếu chạy nhiều polling bot cùng lúc.
// Admin chỉ được bật tối đa 2 bot trong toàn hệ thống (Mini App / Auto-Rải / Bán Acc).
const MAX_ACTIVE_BOTS = Number(process.env.MAX_ACTIVE_BOTS || 2);
function activeBotCount(exceptType = '', exceptId = 0) {
  const mini = db.prepare('SELECT COUNT(*) c FROM bots WHERE active=1' + (exceptType === 'mini' ? ' AND id<>?' : '')).get(...(exceptType === 'mini' ? [exceptId] : [])).c || 0;
  const ar = db.prepare('SELECT COUNT(*) c FROM autorai_bots WHERE active=1' + (exceptType === 'autorai' ? ' AND id<>?' : '')).get(...(exceptType === 'autorai' ? [exceptId] : [])).c || 0;
  const acc = db.prepare('SELECT COUNT(*) c FROM accshop_bots WHERE active=1' + (exceptType === 'accshop' ? ' AND id<>?' : '')).get(...(exceptType === 'accshop' ? [exceptId] : [])).c || 0;
  return mini + ar + acc;
}
function assertCanEnableBot(type, id = 0) {
  const count = activeBotCount(type, Number(id) || 0);
  if (count >= MAX_ACTIVE_BOTS) {
    const err = new Error(`Chỉ được bật tối đa ${MAX_ACTIVE_BOTS} bot cùng lúc. Hãy tắt bớt bot khác trước.`);
    err.status = 400;
    throw err;
  }
}
