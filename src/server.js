...
async function startAccShopBot(row){
  ...
  function adminKeyboard(){ return {inline_keyboard:[
    ...
    [{text:'📢 Thông báo',callback_data:'accadmin:thongbao'},{text:'🗑️ Xoá dữ liệu',callback_data:'accadmin:deletehelp'}],
...
Chọn duyệt nếu đã nhận tiền.`,{reply_markup:{inline_keyboard:[[{text:'✅ Duyệt + cộng tiền',callback_data:`accpay:ok:${topupId}`},{text:'❌ Từ chối',callback_data:`accpay:no:${topupId}`}],[{text:'👤 Xem UID',callback_data:`accpay:user:${uid}`}]]}}); return; }
      ...
      if(st==='wait_listacc' && isAccAdmin(cur,uid)){ const header=state.get(uid+':listHeader'); const parsed=parseListHeader(header||''); if(!parsed){ state.delete(uid); state.delete(uid+':listHeader'); return bot.sendMessage(chat,'Sai mẫu. Dùng: <code>/themacclist Tên danh mục|Giá</code> rồi gửi list <code>user|pass</code>',{parse_mode:'HTML'}); } const c=findCat(parsed.catName); const secrets=extractSecrets((parsed.blob?parsed.blob+'\n':'')+(text||msg.caption||'')); let ok=0; for(const sec of secrets){ db.prepare('INSERT INTO accshop_accounts(bot_id,category_id,title,description,secret,price,status) VALUES(?,?,?,?,?,?,?)').run(row.id,c.id,`Acc ${c.name}`,`Login: ${sec}`,sec,parsed.price,'on'); ok++; } state.delete(uid); state.delete(uid+':listHeader'); await bot.sendMessage(chat,`✅ Đã thêm <b>${ok}</b> acc list vào danh mục <b>${escapeHtml(c.name)}</b>.
...
📦 Kho còn: <b>${catStock(c.id)}</b>`,{parse_mode:'HTML'}); return; }
      ...
      if(['admin','themacc','addacc','themacclist','danhmuc','xoadanhmuc','listdanhmuc','xoaacc','xoakho','thongbao','bank','addzalopay','addmomo','listadmin','listuser','timuser','sethotro','congtien','trutien','duyettien','huytien'].includes(cmd) && !isAccAdmin(cur,uid)){ return; }
...
Gửi tiếp list, mỗi acc dạng <code>user|pass</code>, mỗi dòng hoặc cách nhau bằng dấu cách.`,{parse_mode:'HTML'}); return; }
      ...
      if(cmd==='duyettien'||cmd==='huytien'){ const topupId=Number(arg.replace(/\D/g,'')); if(!topupId) return bot.sendMessage(chat,`Cú pháp: /${cmd} ID_đơn`); const t=db.prepare('SELECT * FROM accshop_topups WHERE bot_id=? AND id=?').get(row.id,topupId); if(!t) return bot.sendMessage(chat,'Không thấy đơn nạp này.'); if(t.status!=='pending') return bot.sendMessage(chat,'Đơn này đã xử lý rồi: '+t.status); if(cmd==='duyettien'){ db.prepare('UPDATE accshop_topups SET status=? WHERE bot_id=? AND id=?').run('approved',row.id,topupId); db.prepare('UPDATE accshop_users SET balance=balance+? WHERE bot_id=? AND tg_user_id=?').run(t.amount,row.id,t.tg_user_id); const u=db.prepare('SELECT balance FROM accshop_users WHERE bot_id=? AND tg_user_id=?').get(row.id,t.tg_user_id); await bot.sendMessage(chat,`✅ Đã duyệt đơn #${topupId} và cộng ${vn(t.amount)}đ.`); try{await bot.sendMessage(t.tg_user_id,`✅ Đơn nạp <b>#${topupId}</b> đã được duyệt.\nĐã cộng: <b>${vn(t.amount)}đ</b>\nSố dư: <b>${vn(u?.balance||0)}đ</b>`,{parse_mode:'HTML'});}catch{} } else { db.prepare('UPDATE accshop_topups SET status=? WHERE bot_id=? AND id=?').run('rejected',row.id,topupId); await bot.sendMessage(chat,`❌ Đã từ chối đơn #${topupId}.`); try{await bot.sendMessage(t.tg_user_id,`❌ Đơn nạp <b>#${topupId}</b> chưa được duyệt. Vui lòng liên hệ hỗ trợ nếu đã chuyển tiền.`,{parse_mode:'HTML'});}catch{} } return; }
      if(cmd==='listuser'){ const users=db.prepare('SELECT * FROM accshop_users WHERE bot_id=? ORDER BY id DESC LIMIT 50').all(row.id); await bot.sendMessage(chat, users.length ? '👥 <b>DANH SÁCH USER</b>\n━━━━━━━━━━━━\n'+users.map(u=>`UID: <code>${u.tg_user_id}</code> · @${escapeHtml(u.username||'')} · ${vn(u.balance)}đ`).join('\n') : 'Chưa có user.', {parse_mode:'HTML'}); return; }
      if(cmd==='timuser'){ const target=Number(arg.replace(/\D/g,'')); if(!target) return bot.sendMessage(chat,'Cú pháp: /timuser UID'); const u=db.prepare('SELECT * FROM accshop_users WHERE bot_id=? AND tg_user_id=?').get(row.id,target); await bot.sendMessage(chat,u?`👤 <b>USER</b>\n━━━━━━━━━━━━\nUID: <code>${u.tg_user_id}</code>\nUsername: @${escapeHtml(u.username||'')}\nTên: ${escapeHtml((u.first_name||'')+' '+(u.last_name||''))}\nSố dư: <b>${vn(u.balance)}đ</b>\nNgày vào: ${escapeHtml(u.created_at||'')}`:'Không thấy user này.',{parse_mode:'HTML'}); return; }
  ...
  bot.on('callback_query', async q=>{
    try{
      ...
      if(data.startsWith('accpay:')){ if(!isAccAdmin(cur,uid)) return; const parts=data.split(':'); const action=parts[1]; if(action==='user') return bot.sendMessage(chat,`UID: <code>${escapeHtml(parts[2]||'')}</code>`,{parse_mode:'HTML'}); const topupId=Number(parts[2]); const t=db.prepare('SELECT * FROM accshop_topups WHERE bot_id=? AND id=?').get(row.id,topupId); if(!t) return bot.sendMessage(chat,'Không thấy đơn nạp này.'); if(t.status!=='pending') return bot.sendMessage(chat,'Đơn này đã xử lý rồi: '+t.status); if(action==='ok'){ db.prepare('UPDATE accshop_topups SET status=? WHERE bot_id=? AND id=?').run('approved',row.id,topupId); db.prepare('UPDATE accshop_users SET balance=balance+? WHERE bot_id=? AND tg_user_id=?').run(t.amount,row.id,t.tg_user_id); const u=db.prepare('SELECT balance FROM accshop_users WHERE bot_id=? AND tg_user_id=?').get(row.id,t.tg_user_id); await bot.sendMessage(chat,`✅ Đã duyệt đơn #${topupId}. Đã cộng ${vn(t.amount)}đ cho UID ${t.tg_user_id}.`); try{ await bot.sendMessage(t.tg_user_id,`✅ Đơn nạp <b>#${topupId}</b> đã được duyệt.\nĐã cộng: <b>${vn(t.amount)}đ</b>\nSố dư hiện tại: <b>${vn(u?.balance||0)}đ</b>`,{parse_mode:'HTML',reply_markup:userKeyboard()}); }catch{} return; } if(action==='no'){ db.prepare('UPDATE accshop_topups SET status=? WHERE bot_id=? AND id=?').run('rejected',row.id,topupId); await bot.sendMessage(chat,`❌ Đã từ chối đơn #${topupId}.`); try{ await bot.sendMessage(t.tg_user_id,`❌ Đơn nạp <b>#${topupId}</b> chưa được duyệt. Nếu đã chuyển tiền, vui lòng bấm Hỗ Trợ.`,{parse_mode:'HTML',reply_markup:userKeyboard()}); }catch{} return; } }
...
Đã giao tự động.`); return; }
      ...
      if(data==='acc:lichsu'){ const rows=db.prepare('SELECT * FROM accshop_topups WHERE bot_id=? AND tg_user_id=? ORDER BY id DESC LIMIT 10').all(row.id,uid); return renderPage(chat, rows.length? '🧾 <b>LỊCH SỬ NẠP</b>\n━━━━━━━━━━━━\n'+rows.map(x=>`#${x.id} ${vn(x.amount)}đ · ${x.status} · ${x.code}`).join('\n'):'🧾 <b>LỊCH SỬ</b>\n━━━━━━━━━━━━\nChưa có lịch sử.',{reply_markup:userKeyboard()},msg); }
