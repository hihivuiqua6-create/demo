...
async function startBot(row) {
  ...
  try {
    ...
    main.on("message", async (msg) => {
      ...
      try {
        const curBot = db.prepare("SELECT sale_bot_enabled FROM bots WHERE id=?").get(fresh.id);
        if (curBot && curBot.sale_bot_enabled !== 0) await handleSaleBotMessage(main, fresh.id, msg);
      } catch (e) { console.error("[sale bot]", e.message); }
    });

    main.onText(/^\/start/, async (msg) => {
      const sale = db.prepare("SELECT sale_bot_enabled FROM bots WHERE id=?").get(fresh.id);
      if (sale && sale.sale_bot_enabled !== 0) return;
      const u = msg.from || {};
