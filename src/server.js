...
CREATE TABLE IF NOT EXISTS security_audit (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  ip TEXT, route TEXT, event TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP
);
`);
...
`);
// v5.7: lệnh điều khiển trực tiếp trong Telegram + danh sách user/channel riêng
safeAlter("ALTER TABLE autorai_bots ADD COLUMN users TEXT DEFAULT '[]'");
safeAlter("ALTER TABLE autorai_bots ADD COLUMN in_user TEXT DEFAULT ''");
safeAlter("ALTER TABLE autorai_bots ADD COLUMN in_channel TEXT DEFAULT ''");
safeAlter("ALTER TABLE autorai_bots ADD COLUMN last_command_at TEXT");
// v5.8: bot-rải only upgrade; mini app giữ nguyên
safeAlter("ALTER TABLE autorai_bots ADD COLUMN random_mode INTEGER DEFAULT 0");
safeAlter("ALTER TABLE autorai_bots ADD COLUMN send_errors TEXT DEFAULT '[]'");
...
function cleanTgItem(raw) {
  const s = String(raw || "").trim();
  if (!s) return "";
  if (/^-?\d+$/.test(s)) return s;
  const m = s.match(/(?:https?:\/\/)?(?:t\.me\/|telegram\.me\/|@)([A-Za-z0-9_]{3,})/i);
  if (m) return "@" + m[1];
  return s.startsWith("@") ? s : "@" + s.replace(/^@+/, "");
}
function arrFromJson(v) { try { const a = JSON.parse(v || "[]"); return Array.isArray(a) ? a : []; } catch { return []; } }
function uniqPush(list, item) {
  const it = cleanTgItem(item);
  if (!it) return list;
  const exists = list.some(x => String(x).toLowerCase() === String(it).toLowerCase());
  return exists ? list : [...list, it];
}
function removeTgItem(list, item) {
  const it = cleanTgItem(item).toLowerCase();
  return list.filter(x => cleanTgItem(x).toLowerCase() !== it);
}
function autoraiHelpText() {
  return "📚 <b>DANH SÁCH LỆNH</b>\n━━━━━━━━━━━━━━━━━━━\n" +
    "<code>/list</code> — hiện full lệnh\n" +
    "<code>/adduser @username</code> — thêm user đã opt-in/đã từng nhắn bot\n" +
    "<code>/addchannel @channel</code> — thêm nhóm/channel bot có quyền gửi\n" +
    "<code>/checkuser</code> — xem user đã add\n" +
    "<code>/checkchannel</code> — xem channel/nhóm đã add\n" +
    "<code>/delete @user_or_channel</code> — xoá khỏi danh sách\n" +
    "<code>/inuser @user</code> — chỉ gửi tới user này\n" +
    "<code>/inchannel @channel</code> — chỉ gửi tới channel/nhóm này\n" +
    "<code>/addcmt nội dung</code> — thêm 1 nội dung rải\n" +
    "<code>/addlist nd1 | nd2 | nd3</code> — thêm nhiều nội dung xoay vòng\n" +
    "<code>/delay1 giây</code> — delay giữa mỗi vòng gửi\n" +
    "<code>/delay2 giây</code> — delay giữa từng user/channel\n" +
    "<code>/run</code> — chạy bot\n" +
    "<code>/stop</code> — dừng bot\n" +
    "<code>/liston</code> — chức năng/trạng thái đang bật\n" +
    "<code>/ai ý tưởng</code> — AI offline gợi ý nội dung quảng bá\n" +
    "<code>/random on/off</code> — bật/tắt random nội dung\n" +
    "<code>/clearlist</code> — xoá toàn bộ nội dung đang rải\n" +
    "<code>/export</code> — xuất cấu hình list/nội dung\n" +
    "<code>/import JSON</code> — nhập cấu hình đã export";
}
function buildTargets(row) {
  const users = arrFromJson(row.users || "[]");
  const channels = arrFromJson(row.channels || "[]");
  const out = [];
  if (row.in_user) out.push(row.in_user);
  else out.push(...users);
  if (row.in_channel) out.push(row.in_channel);
  else out.push(...channels);
  return [...new Set(out.map(cleanTgItem).filter(Boolean))];
}

function aiSuggestContent(idea) {
  const base = String(idea || "dịch vụ/sản phẩm").trim().slice(0, 220) || "dịch vụ/sản phẩm";
  const templates = [
    `🔥 Ưu đãi mới: ${base}\nInbox ngay để được tư vấn chi tiết và nhận hỗ trợ nhanh nhất.`,
    `✅ ${base}\nPhù hợp cho khách cần giải pháp nhanh, rõ ràng, có hướng dẫn đầy đủ. Nhắn mình để xem chi tiết nhé.`,
    `📌 Giới thiệu: ${base}\nCó nhu cầu thì liên hệ, mình hỗ trợ từng bước và giải đáp trước khi dùng.`,
    `🚀 ${base}\nĐang mở nhận khách mới. Ai cần thông tin/bảng giá thì nhắn mình hỗ trợ liền.`
  ];
  return templates.join("\n\n━━━━━━━━━━━━\n\n");
}
function safeExportAutorai(row) {
  return {
    users: arrFromJson(row.users || "[]"),
    channels: arrFromJson(row.channels || "[]"),
    contents: arrFromJson(row.contents || "[]"),
    in_user: row.in_user || "",
    in_channel: row.in_channel || "",
    delay_seconds: Math.max(0, Number(row.delay_seconds) || 0),
    list_delay_seconds: Math.max(0, Number(row.list_delay_seconds) || 0),
    random_mode: row.random_mode ? 1 : 0
  };
}
function pushSendError(rowId, target, err) {
  try {
    const cur = db.prepare("SELECT send_errors FROM autorai_bots WHERE id=?").get(rowId);
    const arr = arrFromJson(cur?.send_errors || "[]");
    arr.push({ at: new Date().toISOString(), target: String(target), error: String(err || '').slice(0, 180) });
    db.prepare("UPDATE autorai_bots SET send_errors=? WHERE id=?").run(JSON.stringify(arr.slice(-30)), rowId);
  } catch {}
}
...
async function startAutoraiBot(row) {
  ...
  try {
    ...
    bot.onText(/^\/start\b/, async (msg) => {
      const text =
        "🤖 <b>AUTO-RẢI BOT</b>\n━━━━━━━━━━━━━━━━━━━\n" +
        "Dùng <code>/key &lt;KEY&gt;</code> để xác thực lần đầu, sau đó dùng <code>/list</code> để xem full lệnh.\n\n" +
        "⚠️ Bot chỉ gửi được tới user đã từng nhắn bot hoặc nhóm/channel mà bot đã được thêm và có quyền gửi.";
    ...
    bot.on("message", async (msg) => {
      ...
      if (/^\/list(?:@\w+)?$/i.test(text) || /^\/help\b/i.test(text)) {
        const extra = (!cur.license_valid || !cur.controller_tg_id) ? "\n\n🔒 Muốn chỉnh/chạy bot thì dùng <code>/key &lt;KEY&gt;</code> trước." : "";
        try { await bot.sendMessage(msg.chat.id, autoraiHelpText() + extra, { parse_mode: "HTML", disable_web_page_preview: true }); } catch {}
        return;
      }
      ...
      db.prepare("UPDATE autorai_bots SET last_command_at=CURRENT_TIMESTAMP WHERE id=?").run(row.id);

      if (/^\/list(?:@\w+)?$/i.test(text) || /^\/help\b/i.test(text)) {
        try { await bot.sendMessage(msg.chat.id, autoraiHelpText(), { parse_mode: "HTML", disable_web_page_preview: true }); } catch {}
        return;
      }
      if (/^\/adduser(?:@\w+)?\s+/i.test(text)) {
        const item = text.replace(/^\/adduser(?:@\w+)?\s+/i, "");
        const users = uniqPush(arrFromJson(cur.users || "[]"), item);
        db.prepare("UPDATE autorai_bots SET users=? WHERE id=?").run(JSON.stringify(users), row.id);
        try { await bot.sendMessage(msg.chat.id, `✅ Đã thêm user: <b>${escapeHtml(cleanTgItem(item))}</b>`, { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/addchannel(?:@\w+)?\s+/i.test(text)) {
        const item = text.replace(/^\/addchannel(?:@\w+)?\s+/i, "");
        const channels = uniqPush(arrFromJson(cur.channels || "[]"), item);
        db.prepare("UPDATE autorai_bots SET channels=? WHERE id=?").run(JSON.stringify(channels), row.id);
        try { await bot.sendMessage(msg.chat.id, `✅ Đã thêm channel/nhóm: <b>${escapeHtml(cleanTgItem(item))}</b>\nℹ️ Nếu là link invite riêng tư, hãy add bot vào nhóm/channel và cấp quyền gửi trước.`, { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/checkuser\b/i.test(text)) {
        const users = arrFromJson(cur.users || "[]");
        try { await bot.sendMessage(msg.chat.id, users.length ? `👤 <b>User đã add</b>\n${users.map(escapeHtml).join("\n")}` : "👤 Chưa có user nào.", { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/checkchannel\b/i.test(text)) {
        const channels = arrFromJson(cur.channels || "[]");
        try { await bot.sendMessage(msg.chat.id, channels.length ? `📣 <b>Channel/nhóm đã add</b>\n${channels.map(escapeHtml).join("\n")}` : "📣 Chưa có channel/nhóm nào.", { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/delete(?:@\w+)?\s+/i.test(text)) {
        const item = text.replace(/^\/delete(?:@\w+)?\s+/i, "");
        const users = removeTgItem(arrFromJson(cur.users || "[]"), item);
        const channels = removeTgItem(arrFromJson(cur.channels || "[]"), item);
        const inUser = cleanTgItem(cur.in_user || "").toLowerCase() === cleanTgItem(item).toLowerCase() ? "" : (cur.in_user || "");
        const inChannel = cleanTgItem(cur.in_channel || "").toLowerCase() === cleanTgItem(item).toLowerCase() ? "" : (cur.in_channel || "");
        db.prepare("UPDATE autorai_bots SET users=?, channels=?, in_user=?, in_channel=? WHERE id=?").run(JSON.stringify(users), JSON.stringify(channels), inUser, inChannel, row.id);
        try { await bot.sendMessage(msg.chat.id, `🗑 Đã xoá nếu tồn tại: <b>${escapeHtml(cleanTgItem(item))}</b>`, { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/inuser(?:@\w+)?\s+/i.test(text)) {
        const item = cleanTgItem(text.replace(/^\/inuser(?:@\w+)?\s+/i, ""));
        const users = uniqPush(arrFromJson(cur.users || "[]"), item);
        db.prepare("UPDATE autorai_bots SET users=?, in_user=? WHERE id=?").run(JSON.stringify(users), item, row.id);
        try { await bot.sendMessage(msg.chat.id, `🎯 Đã bật chế độ chỉ gửi tới user: <b>${escapeHtml(item)}</b>`, { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/inchannel(?:@\w+)?\s+/i.test(text)) {
        const item = cleanTgItem(text.replace(/^\/inchannel(?:@\w+)?\s+/i, ""));
        const channels = uniqPush(arrFromJson(cur.channels || "[]"), item);
        db.prepare("UPDATE autorai_bots SET channels=?, in_channel=? WHERE id=?").run(JSON.stringify(channels), item, row.id);
        try { await bot.sendMessage(msg.chat.id, `🎯 Đã bật chế độ chỉ gửi tới channel/nhóm: <b>${escapeHtml(item)}</b>`, { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/addcmt(?:@\w+)?\s+/i.test(text)) {
        const body = text.replace(/^\/addcmt(?:@\w+)?\s+/i, "").trim();
        const contents = arrFromJson(cur.contents || "[]");
        if (body) contents.push(body);
        db.prepare("UPDATE autorai_bots SET contents=? WHERE id=?").run(JSON.stringify(contents), row.id);
        try { await bot.sendMessage(msg.chat.id, `✅ Đã thêm nội dung. Tổng: <b>${contents.length}</b>`, { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/addlist(?:@\w+)?\s+/i.test(text)) {
        const body = text.replace(/^\/addlist(?:@\w+)?\s+/i, "");
        const arr = parseListPipe(body);
        if (!arr.length) { try { await bot.sendMessage(msg.chat.id, "⚠️ Danh sách rỗng."); } catch {} return; }
        const contents = [...arrFromJson(cur.contents || "[]"), ...arr];
        db.prepare("UPDATE autorai_bots SET contents=? WHERE id=?").run(JSON.stringify(contents), row.id);
        try { await bot.sendMessage(msg.chat.id, `✅ Đã thêm <b>${arr.length}</b> nội dung. Tổng: <b>${contents.length}</b>`, { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/delay1(?:@\w+)?\s+/i.test(text)) {
        const n = Math.max(0, Number(text.replace(/^\/delay1(?:@\w+)?\s+/i, "")) || 0);
        db.prepare("UPDATE autorai_bots SET delay_seconds=? WHERE id=?").run(n, row.id);
        try { await bot.sendMessage(msg.chat.id, `⏱ Delay vòng = <b>${n}s</b>`, { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/delay2(?:@\w+)?\s+/i.test(text)) {
        const n = Math.max(0, Number(text.replace(/^\/delay2(?:@\w+)?\s+/i, "")) || 0);
        db.prepare("UPDATE autorai_bots SET list_delay_seconds=? WHERE id=?").run(n, row.id);
        try { await bot.sendMessage(msg.chat.id, `⏱ Delay từng đích = <b>${n}s</b>`, { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/ai(?:@\w+)?\s+/i.test(text)) {
        const idea = text.replace(/^\/ai(?:@\w+)?\s+/i, "");
        const suggestion = aiSuggestContent(idea);
        try { await bot.sendMessage(msg.chat.id, "🤖 <b>AI gợi ý nội dung</b>\n\n" + escapeHtml(suggestion) + "\n\nDùng <code>/addcmt nội dung</code> hoặc <code>/addlist nd1 | nd2</code> để lưu.", { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/random(?:@\w+)?\s+/i.test(text)) {
        const v = text.replace(/^\/random(?:@\w+)?\s+/i, "").trim().toLowerCase();
        const on = ["on","1","true","bat","bật"].includes(v) ? 1 : 0;
        db.prepare("UPDATE autorai_bots SET random_mode=? WHERE id=?").run(on, row.id);
        try { await bot.sendMessage(msg.chat.id, `🔀 Random nội dung: <b>${on ? "ON" : "OFF"}</b>`, { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/clearlist\b/i.test(text)) {
        db.prepare("UPDATE autorai_bots SET contents='[]' WHERE id=?").run(row.id);
        try { await bot.sendMessage(msg.chat.id, "🧹 Đã xoá toàn bộ nội dung đang rải."); } catch {}
        return;
      }
      if (/^\/export\b/i.test(text)) {
        const fresh = db.prepare("SELECT * FROM autorai_bots WHERE id=?").get(row.id);
        try { await bot.sendMessage(msg.chat.id, `<pre>${escapeHtml(JSON.stringify(safeExportAutorai(fresh), null, 2))}</pre>`, { parse_mode: "HTML" }); } catch {}
        return;
      }
      if (/^\/import(?:@\w+)?\s+/i.test(text)) {
        const raw = text.replace(/^\/import(?:@\w+)?\s+/i, "");
        try {
          const cfg = JSON.parse(raw);
          const users = Array.isArray(cfg.users) ? cfg.users.map(cleanTgItem).filter(Boolean) : arrFromJson(cur.users || "[]");
          const channels = Array.isArray(cfg.channels) ? cfg.channels.map(cleanTgItem).filter(Boolean) : arrFromJson(cur.channels || "[]");
          const contents = Array.isArray(cfg.contents) ? cfg.contents.map(x => String(x).trim()).filter(Boolean).slice(0, 200) : arrFromJson(cur.contents || "[]");
          db.prepare("UPDATE autorai_bots SET users=?, channels=?, contents=?, in_user=?, in_channel=?, delay_seconds=?, list_delay_seconds=?, random_mode=? WHERE id=?")
            .run(JSON.stringify(users), JSON.stringify(channels), JSON.stringify(contents), cleanTgItem(cfg.in_user || ""), cleanTgItem(cfg.in_channel || ""), Math.max(0, Number(cfg.delay_seconds) || 0), Math.max(0, Number(cfg.list_delay_seconds) || 0), cfg.random_mode ? 1 : 0, row.id);
          try { await bot.sendMessage(msg.chat.id, "✅ Import cấu hình thành công."); } catch {}
        } catch (e) { try { await bot.sendMessage(msg.chat.id, "❌ JSON import không hợp lệ."); } catch {} }
        return;
      }
      if (/^\/liston\b/i.test(text)) {
        const fresh = db.prepare("SELECT * FROM autorai_bots WHERE id=?").get(row.id);
        const c = arrFromJson(fresh.contents || "[]");
        const users = arrFromJson(fresh.users || "[]");
        const ch = arrFromJson(fresh.channels || "[]");
        const status = `🟢 <b>LIST ON</b>\n━━━━━━━━━━━━━━━━━━━\n` +
          `Run: <b>${fresh.running ? "ON" : "OFF"}</b>\n` +
          `Nội dung: <b>${c.length}</b>\n` +
          `User: <b>${users.length}</b>${fresh.in_user ? ` | chỉ: ${escapeHtml(fresh.in_user)}` : ""}\n` +
          `Channel: <b>${ch.length}</b>${fresh.in_channel ? ` | chỉ: ${escapeHtml(fresh.in_channel)}` : ""}\n` +
          `Delay1: <b>${fresh.delay_seconds}s</b>\nDelay2: <b>${fresh.list_delay_seconds}s</b>`;
...
setInterval(async () => {
  ...
  for (const r of rows) {
    ...
    const contents = arrFromJson(r.contents || "[]");
    const targets = buildTargets(r);
    if (!contents.length || !targets.length) continue;
    const now = Date.now();
    ...
    (async () => {
      try {
        const content = r.random_mode ? contents[Math.floor(Math.random() * contents.length)] : contents[live.state.idx % contents.length];
        ...
        for (const ch of targets) {
          ...
          try {
            ...
            pushSendError(r.id, target, e.message);
          }
...
function audit(req, event) { try { db.prepare("INSERT INTO security_audit(ip,route,event) VALUES(?,?,?)").run(req.ip || '', req.originalUrl || req.url || '', event); } catch {} }
...
app.get("/api/server/status", adminOnly, (req,res)=>res.json({ok:true, uptime:process.uptime(), live_bots:liveBots.size, autorai_live:liveAutorai.size, db:CONFIG.DB_PATH, public_base_url:CONFIG.PUBLIC_BASE_URL||null, sites_dir:CONFIG.SITES_DIR, security:{default_admin_password: CONFIG.ADMIN_PASSWORD === "admin123456", jwt_secret_env: !!process.env.JWT_SECRET}}));
...
app.get("/api/autorai", anyAuth, (req, res) => {
  ...
  res.json(rows.map(r => {
    ...
    try { o.users = JSON.parse(o.users || "[]"); } catch { o.users = []; }
    return o;
  }));
});
...
app.patch("/api/autorai/:id", anyAuth, async (req, res) => {
  ...
  const { delay_seconds, list_delay_seconds, active, running, owner_seller_id, license_key, regenerate_key, contents, channels, users, in_user, in_channel } = req.body || {};
  ...
  if (Array.isArray(users))
    db.prepare("UPDATE autorai_bots SET users=? WHERE id=?").run(JSON.stringify(users), id);
  if (in_user !== undefined)
    db.prepare("UPDATE autorai_bots SET in_user=? WHERE id=?").run(cleanTgItem(in_user || ""), id);
  if (in_channel !== undefined)
    db.prepare("UPDATE autorai_bots SET in_channel=? WHERE id=?").run(cleanTgItem(in_channel || ""), id);
  if (active !== undefined) {
...
async function startAccountClient(row) {
  ...
  try {
    const client = new TelegramClient(
      ...
      Number(row.api_id),
      String(row.api_hash),
      { connectionRetries: 10, requestRetries: 5, timeout: 30000, useWSS: false, floodSleepThreshold: 60 }
    );
